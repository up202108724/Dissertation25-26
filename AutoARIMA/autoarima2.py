...
class AutoARIMAPipeline:
    ...
    def fit_forecast_lookback(self, df, item_id, store_id, train_size=455, val_size=153, 
        ...
        3) Refit on train to validate on val set.
        4) Refit on train+val.
        5) Recursively forecast 1 step at a time, appending the prediction to the SARIMAX state.
        """
        ...
        val_res = val_model.fit(disp=False, maxiter=maxiter, method='lbfgs')
        
        val_forecast = []
        val_state = val_res

        for i in range(val_size):
            next_exog = X_val[i:i+1] if X_val is not None else None
            pred = val_state.forecast(steps=1, exog=next_exog)[0]
            val_forecast.append(pred)
            val_state = val_state.append([pred], exog=next_exog, refit=False)

        val_rmse = float(np.sqrt(mean_squared_error(y_val, val_forecast)))
        val_mae  = float(mean_absolute_error(y_val, val_forecast))
        print(f"Validation metrics: RMSE={val_rmse:.4f}, MAE={val_mae:.4f}")
        ...
        return rmse, mae, val_rmse, val_mae, order, seasonal_order, forecast, y_train_val, y_test
