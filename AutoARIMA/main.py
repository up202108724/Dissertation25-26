...
def main():
    ...
    for i, prod_row in products.iterrows():
        ...
        # ── Approach 1: Lookback (rolling state propagation) ──────────────
        try:
            t0 = time.time()
            (lb_rmse, lb_mae,
             lb_order, lb_seasonal_order,
             lb_forecast, lb_train_val, _) = pipeline.fit_forecast_lookback(
                df, item_id, store_id,
            ...
            lb_r2   = float(r2_score(y_test, lb_forecast))
            lb_bias = float(np.mean(lb_forecast - y_test))
            _, _, _, _, lb_pocid = compute_metrics(y_test, lb_forecast)
            print(f"  Lookback ({time.time()-t0:.1f}s) → RMSE: {lb_rmse:.4f}, MAE: {lb_mae:.4f}, R2: {lb_r2:.4f}, POCID: {lb_pocid:.4f}")

            row_data.update({
                'lb_rmse': lb_rmse, 'lb_mae': lb_mae,
                'lb_r2': lb_r2, 'lb_bias': lb_bias, 'lb_pocid': lb_pocid,
                'lb_order': str(lb_order),
                'lb_seasonal_order': str(lb_seasonal_order),
            })

            if SAVE_PLOTS:
                plot_results(
                    train=lb_train_val[:TRAIN_SIZE],
                    val=lb_train_val[TRAIN_SIZE:],
                    test=y_test,
                    forecast=lb_forecast,
                    ...
                    val_losses=None,
                    title=(
                        f"AutoARIMA Lookback — Item {item_id}, Store {store_id} "
                        f"| m={best_m} | Order={lb_order}"
                    ),
                    rmse=lb_rmse, mae=lb_mae, bias=lb_bias,
                    score=lb_r2, pocid=lb_pocid,
                    save_path=os.path.join(
                        PLOTS_DIR,
                        f"item{item_id}_store{store_id}_lookback.html",
                    ),
                )

        except Exception as e:
            print(f"  Lookback FAILED: {e}")
