...
# Configuration
# -----------------------------------------------------------------------------
DATA_PATH = os.path.join(script_dir, '..', 'dataset', 'data_smooth_erratic.feather')
DATE_COL = 'date'
TARGET_COL = 'value'
...
def main():
    ...
    results = []
    results_csv = os.path.join(script_dir, 'mlp_smooth_erratic_results.csv')

    # Load already-completed (seed, item_id, store_id) combinations to skip them
    if os.path.exists(results_csv):
        done_df = pd.read_csv(results_csv, usecols=['seed', 'item_id', 'store_id'])
        done_set = set(zip(done_df['seed'], done_df['item_id'], done_df['store_id']))
        print(f"Resuming: {len(done_set)} product/seed combinations already completed.")
    else:
        done_set = set()
    ...
    for seed in seeds:
        for item_id, store_id in products:
            if (seed, item_id, store_id) in done_set:
                print(f"Skipping already completed: Seed {seed}, Item {item_id}, Store {store_id}")
                continue
            ...
            results.append(row)
            # Incremental save — safe even if the run is interrupted
    ...
    print(f"Experiments completed. {len(results)} rows saved to mlp_smooth_erratic_results.csv.")
