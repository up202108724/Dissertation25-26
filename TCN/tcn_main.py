...
# Reuse shared utilities and plots from the LSTM/MLP sibling directories


from plots import plot_results
from utils import generate_exogenous_features, ExogenousScaler
from train import TrainConfig, train_mlp_forecaster
from inference import recursive_inference_dynamic_exog  # TCN-local, leak-safe
# -----------------------------------------------------------------------------
# Configuration
# -----------------------------------------------------------------------------
...
EXOG_COLS = [
    # 1. Cyclical (Lean Base - removed doy, wom, quarter, woy)
    "dow_sin", "dow_cos", 
    "dom_sin", "dom_cos", 
    "month_sin", "month_cos",
    "is_weekend",
    ...
    # 3. Autoregressive (Only features OUTSIDE the 30-day window, or smoothed)
    "lag_364",             # CRITICAL: TCN cannot see a year ago without this
    "rolling_mean_excl_28", # Provides a smoothed monthly macro-trend
    
    # 4. Holidays (Ramps > Binary for Convolutional filters)
    "is_holiday", "is_bridge_day",
    "christmas_proximity", 
    "thanksgiving_proximity", 
    "black_friday_proximity"
]
