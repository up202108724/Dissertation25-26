...
# Original codebase pipeline (used by the 'lstm_simple' baseline head): the
# unmodified LSTM model + single-target dataset + the shared train / inference
# routines that lstm_strategies.py & friends rely on. This runs the initial
# technique exactly as the original scripts do, as a baseline in the grid.
from lstm import LSTM as OriginalLSTM
from dataset import TimeSeriesDataset
from lstm_train import train_model as original_train_model
from lstm_inference import recursive_inference as original_recursive_inference
    ...
    for seed in seeds:
        for item_id, store_id in products:
            ...
            for head in HEADS:
                ...
                if head == 'lstm_simple':
                    # Initial LSTM technique: run the original codebase pipeline
                    # unchanged -> unmodified LSTM model, single-target dataset,
                    # and the shared train_model / recursive_inference routines.
                    model = OriginalLSTM(input_size=input_size, hidden_size=hidden_size,
                                         num_layers=num_layers, dropout=dropout).to(device)
                    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
                    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                        optimizer, mode='min', factor=0.5, patience=PATIENCE // 3)

                    # Original train_model expects single-target (B, 1) batches.
                    simple_train_ds = TimeSeriesDataset(train_scaled, exog_train_scaled, lookback_window)
                    simple_val_ds = TimeSeriesDataset(val_scaled, exog_val_scaled, lookback_window)
                    simple_train_loader = DataLoader(simple_train_ds, batch_size=batch_size, shuffle=False)
                    simple_val_loader = DataLoader(simple_val_ds, batch_size=1, shuffle=False)

                    model, t_losses, v_losses, best_epoch, train_time = original_train_model(
                        seed, EPOCHS, model, simple_train_loader, simple_val_loader, EXOG_COLS,
                        nn.MSELoss(), nn.MSELoss(), optimizer, device, model_path, scheduler, PATIENCE)

                    if os.path.exists(model_path):
                        model.load_state_dict(torch.load(model_path))

                    forecast, infer_time = original_recursive_inference(
                        model, test_start_idx, lookback_window, val_scaled, exog_val_scaled,
                        exog_test_scaled, exog_test, scaler, exog_scaler, df_product, device,
                        EXOG_COLS, forecast_horizon, seed, 'lstm_simple',
                        int(item_id), int(store_id), 'MSELoss', script_dir)
                else:
