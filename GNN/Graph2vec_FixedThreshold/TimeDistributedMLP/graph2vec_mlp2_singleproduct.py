        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                for param_val, window_sz, step_sz, enable_edges, enable_second_degree in iterator:
                    ...
                    inf_result = recursive_inference(
                        model=model,
                        scaler=scaler,
                    ...
                    if SAVE_INFERENCE_GRAPHS_PLOTS and use_embeddings:
                        forecast, inference_nx_graphs = inf_result
                        
                        import sys
                        utils_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'DynamicSimilarities', 'GraphAnalysis'))
                        if utils_path not in sys.path:
                            sys.path.append(utils_path)
                        try:
                            from utils import plot_dynamic_graphs
                        except ImportError:
                            from GNN.DynamicSimilarities.GraphAnalysis.utils import plot_dynamic_graphs
                        ...
                        forecast = inf_result if not (type(inf_result) is tuple) else inf_result[0]
