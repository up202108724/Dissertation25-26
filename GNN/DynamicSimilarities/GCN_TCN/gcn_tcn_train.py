"""
Training loop for the per-step GCN + TCN forecaster.

Same diagnostics + early-stopping logic as the LSTM sibling
(``GCN_TCN/train.py``), with two differences:
    * grad-norm bucket renamed ``grad_norm_lstm`` -> ``grad_norm_tcn``
    * "TCN branch" includes the TCN stack and the linear head (everything
      that is not GCN — ``conv1``/``conv2``/``z_norm``).
"""
...
def train_model(
    ...
    """
    Jointly trains the GCN + TCN end-to-end on the forecasting loss.
    ...
    train_loader      : DataLoader yielding (pyg_batch, ts_batch, y_batch, target_idx, L)
    val_loader        : DataLoader (same format)
    criterion         : training loss  (e.g. MSE)
    criterion2        : validation loss (e.g. MSE / MAE)
    ...
    DIAG_FIELDS = [
        ...
        "grad_norm_gcn", "grad_norm_tcn", "grad_ratio_gcn_tcn",
    ]
    ...
    for epoch in range(epochs):
        ...
        for pyg_batch, ts_batch, y_batch, target_idx, _L in train_loader:
            ...
            with torch.no_grad():
                ...
                for n, p in model.named_parameters():
                    ...
                    if n.startswith("conv1") or n.startswith("conv2") or n.startswith("z_norm"):
                        gn_gcn_sq += g2
                    else:
                        # tcn.*, head.*, head_drop are all temporal-head params
                        gn_tcn_sq += g2
                grad_acc_gcn += gn_gcn_sq ** 0.5
                grad_acc_tcn += gn_tcn_sq ** 0.5
                grad_n += 1
        ...
        avg_gn_tcn = grad_acc_tcn / max(grad_n, 1)
        grad_ratio = avg_gn_gcn / (avg_gn_tcn + 1e-12)
        ...
        if (epoch + 1) % log_every == 0:
            print(
                ...
                f"|grad_gcn|={avg_gn_gcn:.2e} |grad_tcn|={avg_gn_tcn:.2e} "
