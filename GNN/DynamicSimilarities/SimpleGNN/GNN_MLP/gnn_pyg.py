...
class GCNMLPForecaster(nn.Module):
    ...
    A two-layer GCNConv encoder processes the ego-graph, with edge weights
    (similarity/distance scores stored in edge_attr) passed as scalar
    multipliers.  The target-node embedding z (shape B × out_channels) is
    concatenated with the flattened lookback sequence (ts_seq) and fed
    through a multi-layer MLP to produce horizon predictions.
    ...
    def __init__(
        ...
        out_channels: int,          # GCNConv output dim
