"""
Architecture diagram for the GCN + MLP forecaster with **concatenation
fusion** (`GCNMLPForecaster` defined in ``gnn_pyg.py`` of MLPv2_Global).

Per-step pipeline (the network itself predicts 1 step at a time and is
wrapped in a recursive inference loop, see ``recursive_inference_gcn_mlp``
in ``gnninference.py``):
    ...
    ts_seq (B, L, 1+cal_dim) ─► flatten ─► Linear(ts_proj)       |
                                            ...
                                            LayerNorm(ts_enc)    |
                                                  │              |
                                                  ▼              |
                       concat([ LN(z) ‖ LN(ts_enc) ])            |
                       (B, out_channels + ts_proj_dim)           |
                                                  |              |
                                                  v              |
                MLP (mlp_in → 256 → 128 → horizon)               |
                                                  |              |
                                                  v              |
                      ŷ_(t+1) (B, 1, 1) ────────────────────────┘
...
MLP_IN     = GCN_OUT + TS_PROJ_DIM        # 64 + 64 = 128
...
ax.text(10, 12.5,
        "GCN + MLP Forecaster with Concatenation Fusion (GCNMLPForecaster) — Architecture",
        ha="center", va="center", fontsize=15, fontweight="bold")
ax.text(10, 11.95,
        f"window={WINDOW}   lookback={LOOKBACK}   model_horizon={MODEL_HORIZON} "
        f"(recursive × {HORIZON})   GCN({IN_CH}→{GCN_HIDDEN}→{GCN_OUT})   "
        f"ts_proj={TS_INPUT}→{TS_PROJ_DIM}   concat→{MLP_IN}   MLP{MLP_HIDDEN}",
...
# z LayerNorm
z_norm_box = box(ax, (5.3, 4.55), 3.2, 0.80,
                 f"LayerNorm(z)\n(B, {GCN_OUT})",
                 COL_NORM, fontsize=9)
...
ts_norm_box = box(ax, (12.0, 5.45), 4.0, 0.80,
                  f"LayerNorm(ts_enc)\n(B, {TS_PROJ_DIM})",
                  COL_NORM, fontsize=9)
...
# === CONCAT FUSION ======================================================
concat_box = box(ax, (8.0, 3.10), 6.0, 1.10,
                 f"Concat fusion\n[ LayerNorm(z)  ‖  LayerNorm(ts_enc) ]\n"
                 f"(B, {GCN_OUT} + {TS_PROJ_DIM} = {MLP_IN})",
                 COL_CONCAT, fontsize=10)

# arrows from both LN branches into the concat block
arrow(ax, (6.9, 4.55), (9.5, 4.20), rad=-0.15)            # LN(z)   → concat
arrow(ax, (14.0, 5.45), (12.5, 4.20), rad=0.15)           # LN(ts)  → concat
...
arrow(ax, (concat_box[0], 3.10), (centers[0][0] + mlp_w / 2 - 0.2, mlp_y + 0.95),
      rad=-0.25)
...
png_path = os.path.join(out_dir, "gcn_mlp_concat_architecture.png")
pdf_path = os.path.join(out_dir, "gcn_mlp_concat_architecture.pdf")
plt.tight_layout()
