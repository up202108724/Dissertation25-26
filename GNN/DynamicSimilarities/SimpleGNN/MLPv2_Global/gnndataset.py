...
def make_single_windows(
    ...
    for i in range(N):
        ...
        # Read neighbor ts from the explicit node_ts tensor (already scaled with
        # the per-product train-fit scaler when node_scalers was given to the
        # graph builder). Fall back to the legacy x[:graph_window_size] + local
        # min-max convention only when node_ts is missing.
        _dummy_cal_next = np.zeros(cal_dim, dtype=np.float32)
        for node_idx in range(1, n_nodes):
            if hasattr(base_graph, 'node_ts') and base_graph.node_ts is not None:
                neighbor_ts = base_graph.node_ts[node_idx].numpy()
            else:
                orig_feat   = base_graph.x[node_idx].numpy()            # legacy (graph_window_size + 8,)
