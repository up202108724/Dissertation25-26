...
def recursive_inference_gcn_mlp(
    ...
    # --- Leak-safe neighbor values (optional) ---
    # When provided, neighbor cells of df_wide at FUTURE dates of the rolling
    # graph window are replaced, step-by-step, with each neighbor's own
    # local-MLP one-step-ahead prediction. Past-date cells are kept real.
    #   neighbor_models             : dict[pid -> nn.Module] (univariate MLPs)
    #   neighbor_histories_unscaled : dict[pid -> np.ndarray] (unscaled target
    #                                 history up to test_start, used to seed
    #                                 the per-product rolling buffer)
    # The neighbor MLPs are assumed to share the lookback used for the target
    # model and to be wrapped by the same per-product `node_scalers` entry
    # (MinMaxScaler fit on the train slice).
    neighbor_models: dict = None,
    neighbor_histories_unscaled: dict = None,
) -> np.ndarray:
    """
    ...
    # ---- Optional leak-safe neighbor mode setup ----
    use_local_nbr = (
        neighbor_models is not None
        and neighbor_histories_unscaled is not None
        and node_scalers is not None
        and df_wide is not None
    )
    if use_local_nbr:
        df_wide_dyn = df_wide.copy()
        nbr_buffers = {
            pid: list(np.asarray(neighbor_histories_unscaled[pid], dtype=np.float64))
            for pid in neighbor_models.keys()
            if pid != target_id and pid in df_wide_dyn.index
        }
        # Move neighbor models to device once
        for _pid, _m in neighbor_models.items():
            try:
                neighbor_models[_pid] = _m.to(device).eval()
            except Exception:
                pass
    else:
        df_wide_dyn = df_wide
    ...
    with torch.no_grad():
        for i in range(horizon):
            # ---- 0. (Optional) advance each neighbor by ONE step using its
            #         local MLP and overwrite the corresponding future cell of
            #         df_wide_dyn. Past-date cells are untouched.
            if use_local_nbr:
                f_date_i = future_dates[i]
                for pid, lm in neighbor_models.items():
                    if pid == target_id or pid not in nbr_buffers:
                        continue
                    buf = nbr_buffers[pid]
                    if len(buf) < lookback:
                        continue  # not enough history to predict
                    sc_pid = node_scalers.get(pid)
                    if sc_pid is None:
                        continue
                    window_unsc   = np.asarray(buf[-lookback:], dtype=np.float32).reshape(-1, 1)
                    window_scaled = sc_pid.transform(window_unsc).astype(np.float32)
                    x_t = torch.from_numpy(window_scaled).unsqueeze(0).to(device)  # (1, L, 1)
                    y_p = lm(x_t).view(-1)[0].item()
                    next_unsc = float(sc_pid.inverse_transform([[y_p]])[0, 0])
                    if f_date_i in df_wide_dyn.columns:
                        df_wide_dyn.at[pid, f_date_i] = next_unsc
                    nbr_buffers[pid].append(next_unsc)
            ...
            target_preds_for_graph = (
                df_wide_dyn.loc[target_id, window_dates].values.astype(float).copy()
            )
