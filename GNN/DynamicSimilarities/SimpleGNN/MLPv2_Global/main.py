...
# Per-product GCN settings ----------------------------------------------------
# When USE_PER_PRODUCT_GCN is True, one `train_gcn_mlp` model is trained per
# product in df_wide_global (in addition to the per-target GCN models trained
# inside the PRODUCTS_TO_TEST loop). At inference all per-product GCNs run in
# lock-step: every step, each pid produces a prediction from the current
# df_wide_dyn snapshot; only after every pid has produced step-i are the
# updates committed and step i+1 begins. Heavy compute — see bulk_pipeline.py.
USE_PER_PRODUCT_GCN          = True
PER_PRODUCT_GCN_METRIC       = 'spearman'
PER_PRODUCT_GCN_THRESHOLD    = 0.85   # set to None and use *_PERCENTILE instead
PER_PRODUCT_GCN_PERCENTILE   = None
PER_PRODUCT_GCN_WINDOW       = 30
PER_PRODUCT_GCN_STEP         = 1
PER_PRODUCT_GCN_EDGES_STAR   = False
PER_PRODUCT_GCN_2ND_DEG      = False
PER_PRODUCT_GCN_EPOCHS       = 50
PER_PRODUCT_GCN_PATIENCE     = 10
PER_PRODUCT_GCN_LR           = 1e-3
PER_PRODUCT_GCN_WD           = 1e-4
PER_PRODUCT_GCN_BATCH        = 32
# Quick safety knob: limit to first N products for a dry run; None = all.
PER_PRODUCT_GCN_MAX_PIDS     = None


# Shared (global) GCN settings ------------------------------------------------
# When USE_SHARED_GCN is True, ONE GCN is trained on the ego-graphs of all
# products jointly (parameter sharing), then applied per-product at inference
# in the same lock-step loop. Much cheaper than the per-product path and
# typically generalizes better. See shared_pipeline.py.
USE_SHARED_GCN              = False
SHARED_GCN_METRIC           = 'spearman'
SHARED_GCN_THRESHOLD        = 0.85
SHARED_GCN_PERCENTILE       = None
SHARED_GCN_WINDOW           = 30
SHARED_GCN_STEP             = 1
SHARED_GCN_EDGES_STAR       = False
SHARED_GCN_2ND_DEG          = False
SHARED_GCN_EPOCHS           = 30
SHARED_GCN_PATIENCE         = 10
SHARED_GCN_LR               = 1e-3
SHARED_GCN_WD               = 1e-4
SHARED_GCN_BATCH            = 64
SHARED_GCN_HIDDEN_CHANNELS  = 32
SHARED_GCN_OUT_CHANNELS     = 64
SHARED_GCN_TS_PROJ_DIM      = 64
SHARED_GCN_MAX_PIDS         = 50
...
def main():
    ...
    # Seeds for which the per-product GCN bulk pipeline has already been run.
    # Ensures we don't retrain / re-infer the ~1427 GCN-MLPs for every test
    # product inside PRODUCTS_TO_TEST.
    per_product_gcn_done_seeds = set()
    shared_gcn_done_seeds      = set()
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        for seed in SEEDS:
            ...
            # ---------------------------------------------------------------
            # Per-product GCNs (one train_gcn_mlp per product), trained ONCE
            # per seed and inferred in lock-step. Heavy compute — gated by
            # USE_PER_PRODUCT_GCN.
            # ---------------------------------------------------------------
            if USE_PER_PRODUCT_GCN and seed not in per_product_gcn_done_seeds:
                from bulk_pipeline import (
                    BulkGCNConfig,
                    train_gcn_mlps_for_all_products,
                    infer_all_products_lockstep,
                    evaluate_all_products_metrics,
                )
                bulk_cfg = BulkGCNConfig(
                    metric=PER_PRODUCT_GCN_METRIC,
                    threshold=PER_PRODUCT_GCN_THRESHOLD,
                    percentile=PER_PRODUCT_GCN_PERCENTILE,
                    window_size=PER_PRODUCT_GCN_WINDOW,
                    step_size=PER_PRODUCT_GCN_STEP,
                    enable_edges_within_star=PER_PRODUCT_GCN_EDGES_STAR,
                    enable_second_degree=PER_PRODUCT_GCN_2ND_DEG,
                    use_residuals=USE_RESIDUALS,
                    epochs=PER_PRODUCT_GCN_EPOCHS,
                    patience=PER_PRODUCT_GCN_PATIENCE,
                    lr=PER_PRODUCT_GCN_LR,
                    weight_decay=PER_PRODUCT_GCN_WD,
                    batch_size=PER_PRODUCT_GCN_BATCH,
                )
                bulk_save_dir = os.path.join(best_models_seed_dir, 'per_product_gcn')

                if PER_PRODUCT_GCN_MAX_PIDS is not None:
                    full_idx = list(df_wide_global.index)
                    skip_pids_bulk = set(full_idx[PER_PRODUCT_GCN_MAX_PIDS:])
                else:
                    skip_pids_bulk = set()

                bulk_contexts = train_gcn_mlps_for_all_products(
                    df_wide_global=df_wide_global,
                    product_minmax_scalers=product_minmax_scalers,
                    ...
                    dropout=dropout,
                    device=device,
                    save_dir=bulk_save_dir,
                    skip_pids=skip_pids_bulk,
                )

                if bulk_contexts:
                    bulk_forecasts, _ = infer_all_products_lockstep(
                        contexts=bulk_contexts,
                        df_wide_global=df_wide_global,
                        cat_labels_dict=cat_labels_dict,
                        bulk_cfg=bulk_cfg,
                        lookback=lookback_window,
                    ...
                    bulk_csv = os.path.join(
                        script_dir, f'per_product_gcn_results_seed_{seed}.csv'
                    )
                    evaluate_all_products_metrics(
                        forecasts=bulk_forecasts,
                        df_wide_global=df_wide_global,
                        forecast_horizon=forecast_horizon,
                        seed=seed,
                        bulk_cfg=bulk_cfg,
                        out_csv_path=bulk_csv,
                    )
                per_product_gcn_done_seeds.add(seed)

            # ---------------------------------------------------------------
            # Shared (global) GCN: ONE model trained on ego-graphs from all
            # products, applied per-product in lock-step at inference.
            # ---------------------------------------------------------------
            if USE_SHARED_GCN and seed not in shared_gcn_done_seeds:
                from shared_pipeline import (
                    SharedGCNConfig,
                    build_shared_training_data,
                    train_shared_gcn,
                    infer_shared_gcn_lockstep,
                )
                from bulk_pipeline import evaluate_all_products_metrics as _eval_shared
                shared_cfg = SharedGCNConfig(
                    metric=SHARED_GCN_METRIC,
                    threshold=SHARED_GCN_THRESHOLD,
                    percentile=SHARED_GCN_PERCENTILE,
                    window_size=SHARED_GCN_WINDOW,
                    step_size=SHARED_GCN_STEP,
                    enable_edges_within_star=SHARED_GCN_EDGES_STAR,
                    enable_second_degree=SHARED_GCN_2ND_DEG,
                    use_residuals=USE_RESIDUALS,
                    epochs=SHARED_GCN_EPOCHS,
                    patience=SHARED_GCN_PATIENCE,
                    lr=SHARED_GCN_LR,
                    weight_decay=SHARED_GCN_WD,
                    batch_size=SHARED_GCN_BATCH,
                    gcn_hidden_channels=SHARED_GCN_HIDDEN_CHANNELS,
                    gcn_out_channels=SHARED_GCN_OUT_CHANNELS,
                    ts_proj_dim=SHARED_GCN_TS_PROJ_DIM,
                    mlp_hidden_sizes=hidden_sizes,
                    dropout=dropout,
                )
                shared_save_dir = os.path.join(best_models_seed_dir, 'shared_gcn')

                if SHARED_GCN_MAX_PIDS is not None:
                    _full_idx = list(df_wide_global.index)
                    shared_skip = set(_full_idx[SHARED_GCN_MAX_PIDS:])
                else:
                    shared_skip = set()

                shared_pool, shared_contexts = build_shared_training_data(
                    df_wide_global=df_wide_global,
                    product_minmax_scalers=product_minmax_scalers,
                    cat_labels_dict=cat_labels_dict,
                    seed=seed,
                    ...
                    cal_columns=EXOG_COLS,
                    include_cal_lookback=INCLUDE_CAL_LOOKBACK,
                    skip_pids=shared_skip,
                )

                shared_model, _tl, _vl = train_shared_gcn(
                    pool=shared_pool,
                    seed=seed,
                    shared_cfg=shared_cfg,
                    lookback=lookback_window,
                    device=device,
                    save_dir=shared_save_dir,
                    loss_type='mse',
                )

                shared_forecasts, _ = infer_shared_gcn_lockstep(
                ...
                shared_csv = os.path.join(
                    script_dir, f'shared_gcn_results_seed_{seed}.csv'
                )
                _eval_shared(
                    ...
                    seed=seed,
                    bulk_cfg=shared_cfg,
                    out_csv_path=shared_csv,
                )
                shared_gcn_done_seeds.add(seed)
