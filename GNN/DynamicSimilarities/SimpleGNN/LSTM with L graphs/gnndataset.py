...
class SequenceGraphDataset(Dataset):
    """
    Dataset for the GNN/SAGE+LSTM sequential forecaster.

    Each sample:
        y              – (horizon, 1)          target values
        graph_sequence – List[Data] of length L (one graph per lookback step)
        ts_seq         – (L, 1+cal_dim)        explicit LSTM input sequence
    """

    def __init__(self, y: np.ndarray, graph_sequences, ts_seqs: np.ndarray = None):
        self.y = torch.from_numpy(y)           # (N, H, 1)
        self.graph_sequences = graph_sequences  # List[List[Data]], len N
        self.ts_seqs = (torch.from_numpy(ts_seqs) if ts_seqs is not None else None)
    ...
    def __getitem__(self, idx):
        if self.ts_seqs is not None:
            return self.y[idx], self.graph_sequences[idx], self.ts_seqs[idx]
        return self.y[idx], self.graph_sequences[idx]
...
def sequence_graph_collate(batch):
    ...
    Handles both 2-item (y, graph_seq) and 3-item (y, graph_seq, ts_seq) batches.
    ...
    B         : int  (batch size)
    L         : int  (lookback length)
    ts_batch  : (B, L, 1+cal_dim)  only when ts_seqs are present
    """
    y_list   = [item[0] for item in batch]
    seq_list = [item[1] for item in batch]   # List[List[Data]]
    ...
    if len(batch[0]) == 3:
        ts_batch = torch.stack([item[2] for item in batch])   # (B, L, 1+cal_dim)
        return y_batch, pyg_batch, B, L, ts_batch
    return y_batch, pyg_batch, B, L
...
def make_sequence_windows(
    ...
    """
    Build L-graph sequences + ts_seqs for SimpleGNNSeqLSTMForecaster.

    For each sample i, for each lookback step l (0 … L-1):
      * global timestep  t = i + l
      * graph G_t: similarity window of `graph_window_size` days ending at t
      * target-node features: [ts_win(gw) | cal_at_t(cal_dim) | stats_8]
        via compute_target_node_features_seq
      * neighbour features: compute_neighbor_node_features_pure(ts_win, feature_dim)

    ts_seq[i, l, :] = [value_at_t | cal_at_{t+1}]  (same exog-shift as make_single_windows)
    ...
    lookback        : int           L  (LSTM sequence length)
    horizon         : int           H
    graphs          : List[Data]    graphs from neighbourhood_graph (one per timestep)
    graph_window_size : int
    ...
    for i in range(N):
        ...
        # ── ts_seq: [value_t | cal_{t+1}]  for t = i .. i+L-1 ──────────────
        ts_vals = series[i : i + lookback, target_channel]            # (L,)
        t_cal_end = i + lookback + 1
        if t_cal_end <= T:
            cal_shifted = cal[i + 1 : t_cal_end]                     # (L, cal_dim)
        else:
            _avail = cal[i + 1 : T]
            _pad   = np.repeat(cal[T - 1 : T], lookback - len(_avail), axis=0)
            cal_shifted = np.vstack([_avail, _pad]) if len(_avail) > 0 else _pad
        ts_seqs[i] = np.concatenate([ts_vals[:, None], cal_shifted], axis=1)

        # ── L graphs, one per lookback step ──────────────────────────────────
        seq_i = []
        for l in range(lookback):
            t       = i + l
            ...
            # Target node: ts_window ending at t + calendar at t
            start  = max(0, t - graph_window_size + 1)
            ts_win = series[start : t + 1, target_channel]            # ≤ gw values
            cal_at_t = cal[t] if t < T else cal[-1]                   # current-step cal

            x_new[0] = torch.tensor(
                compute_target_node_features_seq(ts_win, cal_at_t, feature_dim),
                dtype=torch.float32,
            )
    ...
    return y_arr, graph_sequences, ts_seqs
