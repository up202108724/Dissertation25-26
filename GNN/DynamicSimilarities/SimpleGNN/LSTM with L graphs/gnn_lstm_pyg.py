...
# ---------------------------------------------------------------------------
# GCN encoder at EACH lookback step  →  z_t concat ts_seq  →  LSTM
# ---------------------------------------------------------------------------

class SimpleGNNSeqLSTMForecaster(nn.Module):
    """
    GCN encoder at every lookback timestep + LSTM forecaster.

    At each LSTM step t the GCN processes the ego-graph whose similarity
    window ends at t (15 prior days).  The target-node embedding z_t is
    concatenated with the explicit time-series features (value_t, cal_{t+1})
    to form the LSTM input:

        lstm_in[:, t, :] = [value_t | cal_{t+1} | z_t]
                            (1 + cal_dim + gnn_out_channels)

    All B×L graphs are batched together (sample-major order) so the full
    GCN runs in a single forward pass.

    Forward inputs
    --------------
    ts_seq    : FloatTensor (B, L, 1 + cal_dim)
    pyg_batch : PyG Batch of B×L graphs, sample-major: [s0_t0, …, s0_t{L-1}, s1_t0, …]
    B         : int  batch size
    ...
    def __init__(
        self,
        in_channels: int,      # node feature dim  (graph_window_size + cal_dim + 8)
        hidden_channels: int,
        out_channels: int,     # GCN output dim per node  (z_t size)
        lstm_ts_input: int,    # explicit ts input dim  (1 + cal_dim)
        lstm_hidden: int,
        lstm_layers: int,
        horizon: int,
        dropout: float = 0.2,
        ...
        self.gnn_drop   = nn.Dropout(p=dropout)
        self.lstm = nn.LSTM(
            lstm_ts_input + out_channels,   # 1 + cal_dim + gnn_out
            lstm_hidden,
            lstm_layers,
            batch_first=True,
            dropout=dropout if lstm_layers > 1 else 0.0,
    ...
    def forward(self, ts_seq, pyg_batch, B: int, L: int):
        """
        ts_seq    : (B, L, 1 + cal_dim)
        pyg_batch : PyG Batch of B×L graphs (sample-major order)
        B, L      : batch size and lookback length
        """
        ew = (pyg_batch.edge_attr.squeeze(1)
              if (pyg_batch.edge_attr is not None and pyg_batch.edge_attr.shape[0] > 0)
              else None)
        ...
        # Extract target-node embedding from every one of the B×L graphs.
        # pyg_batch.ptr[i] = start node index of graph i  →  ptr[:-1] = node 0 of each graph.
        target_indices = pyg_batch.ptr[:-1]               # (B×L,)
        z_flat = h[target_indices]                        # (B×L, out_channels)
        z = z_flat.view(B, L, self.out_channels)          # (B, L, out_channels)

        # Augment ts_seq with GNN embedding at each step
        lstm_in = torch.cat([ts_seq, z], dim=-1)          # (B, L, 1+cal_dim+out_channels)
        lstm_out, _ = self.lstm(lstm_in)                  # (B, L, lstm_hidden)
        out  = self.lstm_drop(lstm_out[:, -1, :])         # (B, lstm_hidden)
        pred = self.head(out)                             # (B, horizon)
        return pred.unsqueeze(-1)                         # (B, horizon, 1)
