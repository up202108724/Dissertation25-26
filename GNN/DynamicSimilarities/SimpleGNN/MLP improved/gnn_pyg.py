...
class GCNMLPForecaster(nn.Module):
    """
    GCN + MLP forecaster — improved fusion.

    Improvements over the base MLP variant:
      1. GCN out_channels is raised to 64 (controlled by gcn_out_channels in
         train.py), giving the graph branch more expressive capacity.
      2. ts_seq is projected from ts_input_size → ts_proj_dim (default 64)
         before concatenation, so both branches have equal dimensionality and
         the MLP cannot trivially ignore the graph embedding z.
      3. LayerNorm is applied to both z and the projected ts representation
         before concat, removing inter-branch scale differences.
    ...
    def __init__(
        ...
        horizon: int,
        dropout: float = 0.2,
        ts_proj_dim: int = 64,      # projects flat_ts to this dim before concat
    ):
        super().__init__()
        ...
        # Improvement 1: project flat ts down to ts_proj_dim
        self.ts_proj = nn.Linear(ts_input_size, ts_proj_dim)

        # Improvement 2: LayerNorm both branches before concat
        self.z_norm  = nn.LayerNorm(out_channels)
        self.ts_norm = nn.LayerNorm(ts_proj_dim)

        # MLP over [z_norm || ts_norm]  –  equal-size branches
        mlp_in = out_channels + ts_proj_dim
        layers = []
        prev = mlp_in
