"""
Architecture diagram for the GCN + LSTM forecaster (`SimpleGNNLSTMForecaster`)
defined in ``gnn_lstm_pyg.py``.
        ...
        +--> c_proj → reshape → (layers, B, lstm_hidden)  ──┤   |
                                          ...
                                          LSTM(ts_seq, (h0, c0))|
                                                ...
                                                Linear(lstm_hidden → 1)
                                                            ...
                                                            v   |
                              y_(t+1) (B, 1, 1) ───────────────┘
                                            (appended to lookback,
                                             ego-graph rebuilt for next step)

The model itself predicts ONE step at a time (horizon=1). Full 152-step
forecast is produced by `recursive_inference_pure_sage` in gnninference.py.
...
ax.text(10, 12.4, "GCN + LSTM Forecaster (SimpleGNNLSTMForecaster) — Architecture",
...
# GCN layers
box(ax, (5.5, 8.6), 3.2, 0.95,
    f"GCNConv₁\nin={IN_CH}  →  out={GCN_HIDDEN}\nadd_self_loops=True",
    COL_GCN, fontsize=9)
box(ax, (5.7, 7.65), 2.8, 0.55,
    "ReLU  +  Dropout(0.2)", "#E8F5E9", fontsize=8)
box(ax, (5.5, 6.55), 3.2, 0.95,
    f"GCNConv₂\nin={GCN_HIDDEN}  →  out={GCN_OUT}\nadd_self_loops=True",
    COL_GCN, fontsize=9)

arrow(ax, (ego_cx + 0.55, ego_cy), (5.5, 9.07), "x, edge_index, edge_weight")
...
z_center = box(ax, (9.4, 6.75), 2.4, 0.85,
               f"z = h[target]\n(B, {GCN_OUT})",
               COL_Z, fontsize=9)
arrow(ax, (8.7, 7.02), (9.4, 7.17))


# === MIDDLE: projection of z to (h0, c0) =================================
h_proj = box(ax, (12.5, 8.10), 3.6, 0.95,
             f"h₀ projection\n"
             f"Linear({GCN_OUT} → {LSTM_HIDDEN}·{LSTM_LAYERS})  →  reshape\n"
...
# y_hat_(t+1) appended to lookback, ego-graph rebuilt, repeat 152 times
...
ax.text(9.5, 2.05,
        f"append ŷ to lookback  →  rebuild ego-graph  →  step t+1\n"
        f"(repeat {HORIZON} times to produce the full horizon)",
        ha="center", va="center", fontsize=9, color="#6A1B9A",
        bbox=dict(facecolor="#F3E5F5", edgecolor="#6A1B9A", boxstyle="round,pad=0.3"))
