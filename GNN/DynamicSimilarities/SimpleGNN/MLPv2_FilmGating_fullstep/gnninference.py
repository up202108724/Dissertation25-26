...
def direct_inference_gcn_mlp(
    model: torch.nn.Module,
    scaler,
    recent_target_unscaled: np.ndarray,    # (lookback,) UNSCALED past target
    recent_exog_scaled: np.ndarray = None,  # (lookback, cal_dim) SCALED exog
    first_future_exog_scaled: np.ndarray = None,  # (cal_dim,) SCALED exog of FIRST forecast step
    device: Optional[str] = None,
    # --- GCN graph parameters ---
    df_wide: pd.DataFrame = None,
    cat_labels: dict = None,
    target_id: int = None,
    metric: str = 'spearman',
    ...
    """Direct multi-step inference for `GCNMLPForecaster`.

    Performs ONE forward pass:
      * Builds a single ego-graph from the last `graph_window_size` past dates
        (no future predictions are fed back -- nothing is recursive).
      * Overrides the target node's features with the full lookback window
        (scaled) + optional cal_lookback + the FIRST forecast step's calendar.
      * The model head outputs all `horizon` predictions in one shot.
    """
    from torch_geometric.data import Batch
    from gnn_pyg import generate_node_features
    ...
    recent_target_unscaled = np.asarray(recent_target_unscaled, dtype=np.float32).ravel()
    lookback = len(recent_target_unscaled)

    cal_dim = 0 if recent_exog_scaled is None else recent_exog_scaled.shape[1]
    cal_window = (np.asarray(recent_exog_scaled, dtype=np.float32).copy()
                  if (include_cal_lookback and cal_dim > 0) else None)

    target_window_scaled = scaler.transform(
        recent_target_unscaled.reshape(-1, 1)
    ).ravel().astype(np.float32)

    window_dates = list(past_dates[-graph_window_size:])
    target_preds_for_graph = (
        df_wide.loc[target_id, window_dates].values.astype(float).copy()
    )
    G_data = build_dynamic_graph_with_calculated_threshold(
    ...
    if first_future_exog_scaled is not None:
        cal_next = np.asarray(first_future_exog_scaled, dtype=np.float32).ravel()
    elif cal_dim > 0 and recent_exog_scaled is not None:
        cal_next = np.asarray(recent_exog_scaled[-1], dtype=np.float32)
    else:
        cal_next = np.zeros(cal_dim, dtype=np.float32) if cal_dim > 0 else None
    ...
    for node_idx in range(1, n_nodes):
        if hasattr(G_data, 'node_ts') and G_data.node_ts is not None:
            ...
            n_min, n_max = neighbor_ts.min(), neighbor_ts.max()
            n_range = n_max - n_min
            neighbor_ts = (((neighbor_ts - n_min) / n_range).astype(np.float32)
                           if n_range > 1e-8 else np.zeros_like(neighbor_ts))
    ...
    model = model.to(device).eval()
    with torch.no_grad():
        y_pred = model(pyg_batch, target_node_indices, ts_seq)   # (1, H, 1)
