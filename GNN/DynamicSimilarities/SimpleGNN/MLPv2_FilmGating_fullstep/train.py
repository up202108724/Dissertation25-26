...
from gnn_pyg import GCNMLPForecaster
...
# ---------------------------------------------------------------------------
# Pure GCN training  (one graph per sample, linear head only)
# ---------------------------------------------------------------------------
...
def train_gcn_mlp(
    ...
    """
    Train either:
      • PureGCNForecaster  – when graphs is not None
    ...
    val_data   = data[val_start_idx - cfg.lookback : val_end_idx]
    ...
    print(f"Mode: {'GCN + MLP' if use_graphs else 'Pure MLP (no graph)'}")

    # ------------------------------------------------------------------ #
    #  Build loaders                                                       #
    # ------------------------------------------------------------------ #
    graphs_train = graphs[:len(train_scaled)] if use_graphs else None
    # Val now includes the last `lookback` rows of train as context (needed so
    # at least ONE direct multi-step window fits when val_size == horizon).
    graphs_val   = (graphs[len(train_scaled) - cfg.lookback :
                           len(train_scaled) + len(val_scaled) - cfg.lookback]
    ...
    from gnn_pyg import generate_node_features as _gnf
    ...
    best_model_path = f'{model_dir}/gcn_product_{product_id}.pth'
