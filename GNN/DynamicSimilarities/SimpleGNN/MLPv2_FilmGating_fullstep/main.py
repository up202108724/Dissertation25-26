...
# Add parent DynamicSimilarities dir so shared deps can be imported
_parent_dir = os.path.normpath(os.path.join(script_dir, '..', '..'))
if _parent_dir not in sys.path:
    sys.path.insert(0, _parent_dir)
# IMPORTANT: keep script_dir at sys.path[0] (force it to the front even if it
# is already present) so the LOCAL mlp.py / train.py / gnninference.py (with
# direct-multi-step head logic) shadow the shared parent copies.
if script_dir in sys.path:
    sys.path.remove(script_dir)
sys.path.insert(0, script_dir)
from mlp import train_mlp_forecaster,direct_inference_mlp
from plots import plot_results
from utils import generate_exogenous_features
from train import TrainConfig, train_gcn_mlp
from gnninference import recursive_inference_gcn_mlp, direct_inference_gcn_mlp
                ...
                for param_val, window_sz, step_sz, enable_edges, enable_second_degree in iterator:
                    ...
                    if not use_embeddings:
                        ...
                        _mlp_cfg = TrainConfig(
                            lookback=lookback_window, horizon=forecast_horizon, batch_size=batch_size,
                        ...
                        # Direct multi-step head: ONE forward pass returns all H steps.
                        # No leakage from future exog because the model never consumes it.
                        mlp_forecast_bl = direct_inference_mlp(
                            model=mlp_model_bl,
                            target_scaler=_mlp_scaler,
                            history_target_unscaled=val[-lookback_window:],
                            history_exog_scaled=exog_val_scaled[-lookback_window:] if EXOG_COLS else None,
                            first_future_exog_scaled=exog_test_scaled[0] if EXOG_COLS else None,
                    ...
                    # Direct multi-step head: ONE forward pass returns all H steps.
                    forecast = direct_inference_gcn_mlp(
                        model=model,
                        scaler=scaler,
                        recent_target_unscaled=val[-lookback_window:],
                        recent_exog_scaled=exog_val_scaled[-lookback_window:] if EXOG_COLS else None,
                        first_future_exog_scaled=exog_test_scaled[0] if EXOG_COLS else None,
                        device=str(device),
