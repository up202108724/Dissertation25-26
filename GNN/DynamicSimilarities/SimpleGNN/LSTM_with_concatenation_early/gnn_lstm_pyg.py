...
class SimpleGNNLSTMForecaster(nn.Module):
    """
    GCN encoder + LSTM forecaster with **EARLY concatenation fusion**.

    Variant of the late-concat design: instead of fusing the GCN target
    embedding `z` with the LSTM's last hidden state at the head, `z` is
    LayerNorm-ed and broadcast across all lookback timesteps, then
    concatenated to ``ts_seq`` along the feature axis. The augmented
    sequence is fed to the LSTM, so the graph context influences every
    gate computation (input/forget/output) rather than only the readout.

    Per-step pipeline (the network predicts 1 step at a time and is wrapped
    in a recursive inference loop, see ``recursive_inference_pure_sage``):

        ego_graph ─► GCNConv₁ → ReLU+Dropout → GCNConv₂ ─► z = h[target]
        z_bcast   = LayerNorm(z).unsqueeze(1).expand(B, L, out_ch)
        ts_seq'   = concat([ ts_seq , z_bcast ], dim=-1)    # (B, L, 1+cal+out_ch)
        ts_seq'   ─► LSTM (zero init) ─► h_T = lstm_out[:, -1, :]
        h_T → Dropout → Linear(horizon) → ŷ_(t+1)  (B, horizon, 1)

    Forward inputs (unchanged from the late-concat variant)
    --------------
    ...
    def __init__(
        self,
        in_channels: int,       # node feature dim (from generate_node_features)
        hidden_channels: int,
        out_channels: int,      # GCN output channels (= z dimension)
        lstm_input_size: int,   # 1 + cal_dim   (BEFORE z is concatenated)
        lstm_hidden: int,
        lstm_layers: int,
        horizon: int,
        dropout: float = 0.2,
        head_hidden: int = 64,  # kept for signature compatibility (unused here)
    ):
        super().__init__()
        # ── GCN encoder ────────────────────────────────────────────────────
        self.conv1 = GCNConv(in_channels, hidden_channels, add_self_loops=True)
        ...
        # ── LayerNorm on z BEFORE broadcasting (equal-scale gating) ────────
        self.z_norm = nn.LayerNorm(out_channels)

        # ── LSTM consumes [ts_seq || z_bcast] each timestep ────────────────
        augmented_input = lstm_input_size + out_channels
        self.lstm = nn.LSTM(
            augmented_input, lstm_hidden, lstm_layers,
            batch_first=True,
            dropout=dropout if lstm_layers > 1 else 0.0,
        )
        self.lstm_drop = nn.Dropout(p=dropout)

        # ── Linear head on h_T only (head_hidden kept for API parity) ──────
    ...
    def forward(self, pyg_batch, target_node_indices, ts_seq):
        ...
        # ── Early fusion: broadcast LN(z) across lookback, concat to ts ───
        B, L, _ = ts_seq.shape
        z_norm  = self.z_norm(z)                         # (B, out_channels)
        z_bcast = z_norm.unsqueeze(1).expand(B, L, self.out_channels)
        ts_aug  = torch.cat([ts_seq, z_bcast], dim=-1)   # (B, L, 1+cal+out_ch)

        # ── LSTM over augmented sequence (zero h0/c0) ──────────────────────
        lstm_out, _ = self.lstm(ts_aug)                  # (B, L, lstm_hidden)
        h_T = self.lstm_drop(lstm_out[:, -1, :])         # (B, lstm_hidden)

        # ── Linear head ────────────────────────────────────────────────────
        pred = self.head(h_T)                            # (B, horizon)
