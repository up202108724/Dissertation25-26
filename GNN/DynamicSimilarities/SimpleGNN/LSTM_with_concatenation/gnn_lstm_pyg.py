...
class SimpleGNNLSTMForecaster(nn.Module):
    """
    GCN encoder + LSTM forecaster with **concatenation fusion**.

    Differs from the original h0/c0-injection variant: the LSTM is run with
    the default zero hidden / cell state, and the GCN target embedding `z`
    is fused with the LSTM's last hidden state by concatenation, mirroring
    the design used in MLPv2_Global (`GCNMLPForecaster`).
    ...
    in a recursive inference loop, see ``recursive_inference_pure_sage``):
        ...
        fused = [ LayerNorm(z)  ‖  LayerNorm(h_T) ]    # (B, out_ch + lstm_hidden)
        fused → Linear → ReLU → Dropout → Linear(horizon)
        → ŷ_(t+1)  (B, horizon, 1)
    ...
    def __init__(
        ...
        horizon: int,
        dropout: float = 0.2,
        head_hidden: int = 64,  # hidden width of the post-concat MLP head
    ):
        super().__init__()
        # ── GCN encoder ────────────────────────────────────────────────────
        self.conv1 = GCNConv(in_channels, hidden_channels, add_self_loops=True)
        ...
        # ── LSTM over the lookback sequence (zero init h0, c0) ─────────────
        self.lstm = nn.LSTM(
        ...
        # ── LayerNorm both branches before concat (equal-scale fusion) ─────
        self.z_norm = nn.LayerNorm(out_channels)
        self.h_norm = nn.LayerNorm(lstm_hidden)

        # ── MLP head on concat([LN(z) || LN(h_T)]) ─────────────────────────
        fused_dim = out_channels + lstm_hidden
    ...
    def forward(self, pyg_batch, target_node_indices, ts_seq):
        ...
        # ── LSTM branch (default zero hidden/cell) ─────────────────────────
        lstm_out, _ = self.lstm(ts_seq)                  # (B, lookback, lstm_hidden)
        h_T = self.lstm_drop(lstm_out[:, -1, :])         # (B, lstm_hidden)

        # ── Concat fusion + MLP head ───────────────────────────────────────
        fused = torch.cat([self.z_norm(z), self.h_norm(h_T)], dim=-1)
        pred  = self.head(fused)                         # (B, horizon)
