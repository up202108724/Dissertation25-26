...
def recursive_inference_mlp_dynamic_exog(
    ...
    history_exog_unscaled: np.ndarray,    # (lookback, n_exog) unscaled exog (training-time generated -> valid)
    ...
    For each forecast step the function recomputes `lag_k` and
    `rolling_mean_excl_w` columns from the running buffer of
    (past + predicted) UNSCALED target values, then scales that exog row with
    `exog_scaler` and runs the model. Mirrors the LSTM baseline's logic but
    for the MLP.
    """
