...
def neighbourhood_graph(product_id, df, metric, metric_type, window_size, compute_func, 
    ...
    for i, start_idx in enumerate(windows_range):
        ...
        x_matrix = []
        node_ts_matrix = []  # explicit per-node SCALED raw ts (n_nodes, window_size)
        ...
        # x stores only the requested features; raw ts is stored separately in
        # data.node_ts (see below). This removes the implicit "first window_size
        # entries of x are raw ts" convention.
        _storage_feats = list(_feats)
        for n_id in graph_nodes:
            row = window_data.loc[n_id].values.astype(np.float32)
            # Scale this node's window using its own train-fit scaler (same family
            # as the target node). If no scaler is provided, fall back to local
            # min-max normalisation for backward compatibility.
        ...
        data.node_ts  = node_ts_tensor              # (n_nodes, window_size) scaled
...
def parse_dynamic_exog_cols(exog_cols):
    ...
    Used by leak-safe recursive inference to recompute dynamic exogenous
    features from a running buffer of past + predicted targets.
    """
    lag_cols = {}
    roll_cols = {}
