...
def recursive_inference_graphsage_mlp(
...
def recursive_inference_no_graph(
    ...
    """
    Recursive inference for GraphSAGEMLPForecaster with no graph (dummy single-node graph).
    Mirrors the no-graph training path: a single isolated node with no edges is used
    so the SAGEConv passes through only the self-loop, and the MLP branch drives predictions.
    """
    ...
    # Infer node feature dimension from the model's first SAGEConv layer
