...
class GraphSAGEMLPForecaster(nn.Module):
    """
    GraphSAGE + MLP forecaster.

    The GraphSAGE encoder processes the ego-graph and produces a target-node
    embedding z (shape B × sage_out_ch).  z is concatenated with the flattened
    lookback sequence (ts_seq), and the result is processed by a multi-layer
    MLP to produce horizon predictions.

    This is the MLP analogue of GraphSAGELSTMForecaster: instead of projecting
    z to LSTM initial states and running the LSTM sequentially, z is directly
    prepended to the flat ts_seq vector and fed through FC layers.
    ...
    def __init__(
        ...
        hidden_channels: int,       # SAGEConv hidden dim
        out_channels: int,          # sage_out_ch – SAGEConv output dim
