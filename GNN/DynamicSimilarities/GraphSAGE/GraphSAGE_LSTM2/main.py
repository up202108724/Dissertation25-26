...
from lstm import LSTM, TimeSeriesDataset, train_lstm as train_lstm_base, recursive_inference_lstm
from mlp import TrainConfig as MlpTrainConfig, train_mlp_forecaster, recursive_inference_mlp

from plots import plot_results
from utils import generate_exogenous_features
from train import TrainConfig, train_pure_sage
from graphsageinference import recursive_inference_pure_sage
...
def main():
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        for seed in SEEDS:
            ...
            # ── LSTM Baseline (trained once per seed, before config loop) ────────────────
            print(f"\n{'='*60}")
            print(f"Running LSTM Baseline (seed={seed})")
            print(f"{'='*60}")
            _lstm_input      = 1 + len(EXOG_COLS_LSTM)
            _ts_train_ds     = TimeSeriesDataset(train_scaled, exog_lstm_train_scaled, lookback_window)
            _ts_val_ds       = TimeSeriesDataset(val_scaled,   exog_lstm_val_scaled,   lookback_window)
            ...
            lstm_model       = LSTM(input_size=_lstm_input, hidden_size=LSTM_HIDDEN, num_layers=LSTM_LAYERS, dropout=dropout).to(device)
            lstm_model_path  = os.path.join(best_models_seed_dir, f'lstm_product_{product_id}.pth')
            lstm_optimizer   = torch.optim.Adam(lstm_model.parameters(), lr=LEARNING_RATE, weight_decay=1e-3)
            _lstm_crit       = nn.MSELoss()
            lstm_model, lstm_t_losses, lstm_v_losses, _ = train_lstm_base(
                seed=seed, epochs=EPOCHS, model=lstm_model,
                train_loader=_ts_train_loader, val_loader=_ts_val_loader,
                exog_cols=EXOG_COLS_LSTM, criterion=_lstm_crit, criterion2=_lstm_crit,
            ...
            lstm_forecast = recursive_inference_lstm(
                model=lstm_model, test_start_idx=test_start_idx, seq_length=lookback_window,
                val_scaled=val_scaled, exog_val_scaled=exog_lstm_val_scaled,
                exog_test_scaled=exog_lstm_test_scaled,
                exog_test=exog_lstm_test_raw,
                scaler=scaler, exog_scaler=exog_lstm_scaler, df_product=df_product_lstm,
                device=device, exog_cols=EXOG_COLS_LSTM,
            ...
            # ── MLP Baseline (trained once per seed, before config loop) ────────────────
            print(f"\n{'='*60}")
            print(f"Running MLP Baseline (seed={seed})")
            print(f"{'='*60}")
            _mlp_scaler = MinMaxScaler()
            _mlp_cfg = MlpTrainConfig()
            _mlp_cfg.lookback = lookback_window
            _mlp_cfg.horizon = 1
            _mlp_cfg.batch_size = batch_size
            _mlp_cfg.train_size = train_size
            _mlp_cfg.val_size = val_size
            _mlp_cfg.lr = LEARNING_RATE
            _mlp_cfg.epochs = EPOCHS
            _mlp_cfg.device = str(device)
            mlp_model_bl, _mlp_scaler, mlp_t_losses_bl, mlp_v_losses_bl, _ = train_mlp_forecaster(
                ...
                hidden_sizes=hidden_sizes, target_col=TARGET_COL, exog_cols=EXOG_COLS_MLP,
                test_size=forecast_horizon,
            )
            _mlp_recent = np.column_stack([
                val[-lookback_window:].reshape(-1, 1), exog_val_scaled[-lookback_window:]
            ]) if EXOG_COLS_MLP else val[-lookback_window:].reshape(-1, 1)
            mlp_forecast_bl = recursive_inference_mlp(
                model=mlp_model_bl, scaler=_mlp_scaler,
                recent_history=_mlp_recent,
                future_exog=exog_test_scaled if EXOG_COLS_MLP else np.zeros((forecast_horizon, 0)),
