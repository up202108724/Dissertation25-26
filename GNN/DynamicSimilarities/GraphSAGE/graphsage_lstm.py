...
from graphsagedataset import JointSAGETimeSeriesDataset, joint_collate_fn
from lstm import GraphSAGELSTM
...
from utils import infer_metric_type
        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                for param_val, window_size, step_size, enable_edges, enable_second_degree in iterator:
                    ...
                    # TODO: Implement neighbourhood_graph & convert_nx_to_pyg loop here
                    # all_pyg_graphs = list_of_pyg_graphs_for_each_timestep
                    
                    input_size = 1 + (len(EXOG_COLS) if EXOG_COLS else 0)

                    # Initialize missing vars for testing script logic
                    if not use_embeddings:
                        dummy_train_graphs = [] # Replace with actual list of None for no_emb baseline
                        dummy_val_graphs = []

                    train_dataset = JointSAGETimeSeriesDataset(
                        target_data=train_scaled, 
                        exog_data=exog_train_scaled if EXOG_COLS and len(EXOG_COLS) > 0 else None, 
                        seq_length=seq_length,
                        pyg_graphs_list=dummy_train_graphs, # Replace with actual full sequence train slice
                        target_item_name=product_id
                    )
                    use_pin_memory = torch.cuda.is_available()
                    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=use_pin_memory, collate_fn=joint_collate_fn)
                
                    val_dataset = JointSAGETimeSeriesDataset(
                        target_data=val_scaled, 
                        exog_data=exog_val_scaled if EXOG_COLS and len(EXOG_COLS) > 0 else None, 
                        seq_length=seq_length,
                        pyg_graphs_list=dummy_val_graphs, # Replace with actual full sequence val slice
                        target_item_name=product_id
                    )
                    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=use_pin_memory, collate_fn=joint_collate_fn)
                    ...
                    # Instantiate the new GraphSAGE joint model
                    model = GraphSAGELSTM(sage_in_channels=23, sage_hidden=64, sage_out=32, 
                                          lstm_exog_features=len(EXOG_COLS) if EXOG_COLS else 0, 
                                          lstm_hidden=HIDDEN_SIZE, lstm_layers=NUM_LAYERS, 
                                          lstm_dropout=DROPOUT).to(device)
