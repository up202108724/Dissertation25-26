...
def train_model(seed, epochs, model, train_loader, val_loader, exog_cols, criterion, criterion2, optimizer, device, best_model_path, scheduler=None, patience=10):
    ...
    for epoch in range(epochs):
        model.train()
        epoch_loss = 0
        for batch_idx, (batch_x, pyg_graph_batch, target_indices, batch_y) in enumerate(train_loader):
            batch_x, batch_y = batch_x.to(device), batch_y.to(device)
            pyg_graph_batch = pyg_graph_batch.to(device)
            
            optimizer.zero_grad()
            outputs = model(batch_x, pyg_graph_batch, target_indices)
        ...
        with torch.no_grad():
            for batch_idx, (batch_x, pyg_graph_batch, target_indices, batch_y) in enumerate(val_loader):
