...
def train_mlp_forecaster(
    ...
    # scaler is already fit on the proper training slice in main.py — use transform only.
    train_scaled = train_data.copy()
    train_target = train_data[:, target_channel:target_channel+1]
    train_scaled[:, target_channel:target_channel+1] = scaler.transform(train_target)
    # Scale exog columns to match inference-time scaling.
    if exog_scaler is not None and exog_cols:
        exog_col_indices = [i for i in range(train_data.shape[1]) if i != target_channel]
        exog_df = pd.DataFrame(train_data[:, exog_col_indices], columns=exog_cols)
        train_scaled[:, exog_col_indices] = exog_scaler.transform(exog_df, exog_cols).values
    ...
    if exog_scaler is not None and exog_cols:
        val_exog_df = pd.DataFrame(val_context_data[:, exog_col_indices], columns=exog_cols)
        val_scaled[:, exog_col_indices] = exog_scaler.transform(val_exog_df, exog_cols).values
