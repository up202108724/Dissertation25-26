...
class SimpleGCNLSTMForecaster(nn.Module):
    ...
    def forward(self, pyg_batch, target_node_indices, ts_seq):
        ...
        # Diagnostic ablation: zero out the GCN contribution so the LSTM
        # receives only [target ‖ exog ‖ 0…0].  Used to confirm whether
        # the graph branch carries any predictive signal.
        if self.ablate_z:
