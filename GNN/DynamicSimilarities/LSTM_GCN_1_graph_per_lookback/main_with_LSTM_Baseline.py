...
from lstm import LSTM as PlainLSTM
from lstm_train import train_model as lstm_train_fn
from lstm_dataset import TimeSeriesDataset as LSTMDataset
...
# ── Diagnostic switches ───────────────────────────────────────────────────
# When True the GCN embedding `z` is zeroed-out before being concatenated
# with the temporal features — used to confirm whether the graph branch
# carries any predictive signal (RMSE of ablation vs RMSE of baseline).
ABLATE_Z       = True
...
# ──────────────────────────────────────────────────────────────────────────
# Plain-LSTM helpers (used when ABLATE_Z=True)
# ──────────────────────────────────────────────────────────────────────────
from lstm_inference import recursive_inference as lstm_recursive_inference
...
def main():
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        # Raw (unscaled) test exog — needed by lstm_recursive_inference for dynamic
        # lag / rolling-mean recomputation at each forecast step.
        exog_test = df_p[EXOG_COLS][test_slice].values if EXOG_COLS else None
        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                # When ablating, z is always zeroed so the threshold has no effect on
                # the trained model. Collapse to a single run to avoid wasted compute.
                if ABLATE_Z:
                    params = params[:1]
                ...
                for param_val, window_size, step_size, enable_edges, enable_second_degree in iterator:
                    ...
                    if ABLATE_Z:
                        # ══════════════════════════════════════════════════════
                        # LSTM Baseline path — no graphs, plain LSTM model
                        # Steps 1-2 (graph build / PyG alignment) are skipped.
                        # ══════════════════════════════════════════════════════
                        fixed_threshold = None
                        inf_threshold   = None

                        # ── 3b. Datasets / loaders (plain LSTM) ──────────────
                        ...
                        lstm_train_loader = DataLoader(
                            lstm_train_ds, batch_size=BATCH_SIZE, shuffle=False,
                            pin_memory=use_pin_memory,
                        )
                        lstm_val_loader = DataLoader(
                            lstm_val_ds, batch_size=BATCH_SIZE, shuffle=False,
                            pin_memory=use_pin_memory,
                        )
                        ...
                        best_models_dir = os.path.join(
                            best_models_seed_dir, "lstm_baseline",
                        )
                        os.makedirs(best_models_dir, exist_ok=True)
                        base_name       = f"best_lstm_baseline_{product_id}_seed_{seed}"
                        ...
                        if os.path.exists(best_model_path) and os.path.exists(history_path):
                            print(f"Loading existing LSTM baseline from {best_model_path}...")
                            ...
                            # lstm_train_fn always saves best weights; we use a staging path
                            # so that SAVE_MODELS=False still gives us the best-epoch weights.
                            _ckpt_path = os.path.join(best_models_dir,
                                                       f"{base_name}_staging.pth")
                            model, train_losses, val_losses, best_epoch, train_time = lstm_train_fn(
                                ...
                                best_model_path=_ckpt_path,
                                scheduler=scheduler, patience=PATIENCE,
                            )
                            # Restore best-epoch weights saved during training
                            if os.path.exists(_ckpt_path):
                                model.load_state_dict(
                                    torch.load(_ckpt_path, map_location=device)
                                )
                            if SAVE_MODELS:
                                if os.path.exists(_ckpt_path):
                                    os.replace(_ckpt_path, best_model_path)
                        ...
                        # ── 7b. Recursive inference (plain LSTM) ──────────────
                        print("Running LSTM Baseline Inference...")
                        forecast, _ = lstm_recursive_inference(
                            model=model,
                        ...
                        model.ablate_z = False   # always False in this branch
                        criterion  = nn.MSELoss()
                        criterion2 = nn.MSELoss()
                        optimizer  = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE)
                        scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(
                        ...
                        if os.path.exists(best_model_path) and os.path.exists(history_path):
                            ...
                            diag_csv_path = os.path.join(SCRIPT_DIR, DIAG_CSV_NAME)
