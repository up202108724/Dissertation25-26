...
def main():
    ...
    product_id, store_id = _resolve_selected_product(shared)
    prod = prepare_product(shared, product_id, store_id)
    if prod is None:
        raise RuntimeError(f"Product {(product_id, store_id)} has insufficient history.")

    out_dir = os.path.join(OUT_BASE, f"product_{product_id}_{store_id}", f"seed_{SELECTED_SEED}")
    os.makedirs(out_dir, exist_ok=True)
    print(f"\nAnalysing product={product_id} store={store_id} seed={SELECTED_SEED}")
