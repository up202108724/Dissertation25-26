...
def get_graph2vec_embeddings(graphs, dimensions=20, wl_iterations=2, workers=1, epochs=100, min_count=1, window=0, seed=42, train_graph_count=None):
    """
    Generates Graph2Vec embeddings for a sequence of graphs.
    Returns the graph embeddings.

    Two methodological safeguards (see dissertation review notes):

    * Train-only fit: the Doc2Vec/WL model is fitted ONLY on graphs whose
      sliding window lies entirely in the training period (`train_graph_count`).
      Fitting on the full timeline lets val/test graph structure leak into the
      learned substructure/document vectors (transductive leakage).

    * Consistent embedding mechanism: ALL embeddings (train, val and test) are
      produced via ``model.infer`` (Doc2Vec ``infer_vector``) — the exact same
      path used during recursive inference. Previously training used the fitted
      ``model.dv[tag]`` vectors while inference used ``infer_vector``; those two
      live in different regions/scales, so the head was trained and evaluated on
      mismatched embedding distributions.
    """
    ...
    # Restrict the fit corpus to training-period graphs only.
    if train_graph_count is None:
        fit_graphs = valid_graphs
        print("WARNING: train_graph_count not provided — Graph2Vec is being fit "
              "transductively over the full timeline (val/test structure leaks "
              "into the embedding model).")
    else:
        fit_graphs = [G for G, idx in zip(valid_graphs, valid_indices) if idx < train_graph_count]
        if not fit_graphs:  # degenerate guard (e.g. all training graphs empty)
            fit_graphs = valid_graphs

    print(f"Fitting Graph2Vec on {len(fit_graphs)} train-period graphs; "
          f"inferring embeddings for {len(valid_graphs)} valid graphs out of {len(graphs)}...")

    # Initialize and fit Graph2Vec on the training graphs only
    ...
    model.fit(fit_graphs)

    # Produce embeddings for EVERY valid graph through infer_vector (same path as
    # recursive inference), not the transductively-fitted model.dv[tag] vectors.
    emb = model.infer(valid_graphs)
...
def load_or_generate_embeddings(product_id, metric, window_size, step_size, threshold, percentile, 
    ...
    try:
        ...
        # Graph window index g spans days [g, g+window_size-1]; it is a training
        # graph iff it ends strictly before the validation split, i.e.
        # g < train_end_idx - window_size + 1.
        train_graph_count = max(1, train_end_idx - window_size + 1) if train_end_idx else None
