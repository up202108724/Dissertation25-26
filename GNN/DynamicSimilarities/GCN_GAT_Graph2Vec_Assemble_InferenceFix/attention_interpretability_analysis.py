...
# ════════════════════════════════════════════════════════════════════════════
#  One (product, seed) attention analysis
# ════════════════════════════════════════════════════════════════════════════
def run_one(shared, product_id, store_id, seed):
    """Train the GAT for one (product, seed) and snapshot its attention.

    ``R.SELECTED_SEED`` / ``R.SELECTED_PRODUCT`` are set by ``main`` before this
    call so the seed-dependent helpers (build_graphs_for_variant, training) use
    the current pair.
    """
...
# ════════════════════════════════════════════════════════════════════════════
#  Main — loop over the shared (product, seed) pairs
# ════════════════════════════════════════════════════════════════════════════
def main():
    ...
    # Reuse the representation-quality module's pair resolution so both analyses
    # iterate over exactly the same (item_id, store_id, seed) triples.
    triples = R.resolve_pairs(shared)
    print(f"Running GAT attention analysis on {len(triples)} (product, seed) pair(s).")

    for i, (product_id, store_id, seed) in enumerate(triples, 1):
        # Mutate the globals the seed-dependent helpers read (in R's namespace).
        R.SELECTED_SEED = seed
        R.SELECTED_PRODUCT = (product_id, store_id)
