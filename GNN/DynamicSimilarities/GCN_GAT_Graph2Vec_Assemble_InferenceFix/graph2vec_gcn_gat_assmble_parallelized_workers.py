...
from gcn_lstm_dataset import (
    ...
    nx_window_to_pyg as nx_window_to_pyg_lstm,
)
from gat_tdmlp_dataset import (
    ...
    nx_window_to_pyg as nx_window_to_pyg_mlp,
)
...
LEARNING_RATE_LSTM = 0.001   # gcn_lstm / graph2vec_lstm / lstm ablation + LSTM baseline
LEARNING_RATE_MLP  = 0.0001   # gcn_mlp  / gat_mlp / graph2vec_mlp / mlp ablation + MLP baseline
# GAT's multi-head attention destabilises at the LSTM-head default of 1e-3 (it was
# the worst-ranked LSTM variant), so gat_lstm trains at a gentler rate — see
# resolve_learning_rate().  gat_mlp keeps the MLP-head rate (1e-4), which is
# already low enough, so it is intentionally NOT overridden here.
LEARNING_RATE_GAT_LSTM = 0.0005   # gat_lstm only
HIDDEN_SIZE    = 32
NUM_LAYERS     = 1
DROPOUT_LSTM   = 0.0          # LSTM-head variants + LSTM baseline
DROPOUT_MLP    = 0.2          # MLP-head variants + MLP baseline
# ReduceLROnPlateau toggle per head (applies to the gcn/gat/graph2vec variants).
# The MLP head is trained at a fixed LR (scheduler disabled); flip to True to
# re-enable.  The MLP baseline has no scheduler regardless.
...
def resolve_learning_rate(variant):
    """Per-variant learning rate.

    Defaults to the head rate (LSTM=1e-3, MLP=1e-4).  ``gat_lstm`` is the lone
    exception: its multi-head attention is sensitive to the LSTM-head default
    and trains at the gentler ``LEARNING_RATE_GAT_LSTM``.  ``gat_mlp`` is left
    on the MLP rate, which is already lower than the GAT override.
    """
...
PARALLEL = True
PARALLEL_MODE = "seed"   # "seed" or "product"
MAX_CONCURRENT = 6
...
# Narrow the product list to the leftover of one or more previous splits, then
# re-shard across N_PRODUCT_WORKERS so the remaining work fills every core
# instead of staying pinned to a few workers.  Each stage ``(num_workers,
# [worker_ids])`` is applied IN ORDER, each narrowing the list further via
# round-robin reconstruction.  Deterministic, so every new worker computes the
# identical subset before re-sharding.
#   (6, [2, 3]) → keep workers 2 and 3 of the first 6-way split  (20 products).
#   (6, [3])    → then keep only worker 3 of that subset          (3 products),
#                 used after worker 3 alone was interrupted mid-run.
# Resume skips units already finished, so only the ~23 pending units actually
# run.  Set REDISTRIBUTE_STAGES = None (or []) for a normal full run.
REDISTRIBUTE_STAGES = [
    (6, [2, 3]),
    (6, [3]),
]
...
def main():
    ...
    worker_csv_suffix = f"_w{worker_id}" if (num_workers > 1 or is_seed_worker) else ""
    ...
    for (product_id, store_id), seed in itertools.product(PRODUCTS_TO_TEST, seeds_to_run):
        ...
        # ── Outer-loop skip: bypass data loading if every expected result is done ──
        if _done_keys:
            def _check_all_done_for_product():
                for _c in grid_configs:
                    ...
                    for _bv in (
                        ...
                        _bk = (str(product_id), str(store_id), str(seed), _m, _bv,
                               "", "", "", "", "", "", "", "")
                        if _bk not in _done_keys:
                            return False
                return True
            if _check_all_done_for_product():
                print(f"[RESUME] All configs already done for product={product_id} "
                      f"store={store_id} seed={seed} — skipping.")
                units_done += 1
                continue
        ...
        if RUN_LSTM_BASELINE:
            ...
            # Loss curves are persisted alongside the checkpoint (mirrors the
            # variant *_history.pkl) so the overlay plot still shows train/val
            # loss when the baseline is reloaded instead of retrained.
            bl_history_path = bl_model_path.replace(".pth", "_history.pkl")
            if os.path.exists(bl_model_path):
                ...
                if os.path.exists(bl_history_path):
                    with open(bl_history_path, 'rb') as _bl_hf:
                        _bl_hist = pickle.load(_bl_hf)
                    _bl_t_losses = _bl_hist.get('train_losses', [])
                    _bl_v_losses = _bl_hist.get('val_losses', [])
            else:
                ...
                bl_model.load_state_dict(torch.load(bl_model_path, map_location=device))
                with open(bl_history_path, 'wb') as _bl_hf:
                    pickle.dump({'train_losses': _bl_t_losses,
                                 'val_losses': _bl_v_losses}, _bl_hf)
        ...
        if RUN_MLP_BASELINE:
            ...
            # Same checkpoint path that train_mlp_forecaster writes internally:
            # best_models/seed_{seed}/mse/mlp_product_{product_id}_{store_id}.pth
            # (relative to cwd, matching the function's own save logic).
            mlp_bl_model_path = os.path.abspath(os.path.join(
                'best_models', f'seed_{seed}', 'mse',
                f'mlp_product_{product_id}_{store_id}.pth',
            ))
            # Loss curves are persisted alongside the checkpoint (mirrors the
            # variant *_history.pkl) so the overlay plot still shows train/val
            # loss when the baseline is reloaded instead of retrained.
            mlp_bl_history_path = mlp_bl_model_path.replace(".pth", "_history.pkl")
            if os.path.exists(mlp_bl_model_path):
                print(f"Loading existing MLP baseline from {mlp_bl_model_path}...")
                _c_in = 1 + len(EXOG_COLS_MLP)
                mlp_model = TimeDistributedMLPForecaster(
                    in_channels=_c_in,
                    hidden_sizes=MLP_HIDDEN_SIZES,
                    dropout=DROPOUT_MLP,
                    activation="relu",
                ).to(device)
                mlp_model.load_state_dict(torch.load(mlp_bl_model_path, map_location=device))
                mlp_model.eval()
                if os.path.exists(mlp_bl_history_path):
                    with open(mlp_bl_history_path, 'rb') as _mlp_hf:
                        _mlp_hist = pickle.load(_mlp_hf)
                    _mlp_t_losses = _mlp_hist.get('train_losses', [])
                    _mlp_v_losses = _mlp_hist.get('val_losses', [])
                else:
                    _mlp_t_losses, _mlp_v_losses = [], []
            else:
                ...
                os.makedirs(os.path.dirname(mlp_bl_history_path), exist_ok=True)
                with open(mlp_bl_history_path, 'wb') as _mlp_hf:
                    pickle.dump({'train_losses': _mlp_t_losses,
                                 'val_losses': _mlp_v_losses}, _mlp_hf)
        ...
        for config in all_configs:
            ...
            if _baseline_rows:
                ...
                with open(baseline_csv_path, 'a', newline='') as _bl_csvfile:
                    ...
                    for _mv, _r, _m, _b, _s, _p in _baseline_rows:
                        _bl_key = (str(product_id), str(store_id), str(seed),
                                   metric, _mv, "", "", "", "", "", "", "", "")
                        if _bl_key in _done_keys:
                            continue
                        _bl_writer.writerow([
            ...
            for (model_variant, ablate_z, param_val, window_size, step_size,
                ...
                _ct_str = str(current_threshold)  if current_threshold  is not None else ""
                _cp_str = str(current_percentile) if current_percentile is not None else ""
                _resume_key = (
                    str(product_id), str(store_id), str(seed), metric, model_variant,
                    str(window_size), str(step_size), _ct_str, _cp_str,
                    str(enable_edges), str(enable_second_degree), str(ablate_z),
                    node_feature_mode,
                )
                if _resume_key in _done_keys:
                    print(f"[RESUME] Skipping {model_variant} ablate_z={ablate_z} "
                          f"product={product_id} store={store_id} seed={seed}")
                    continue
                ...
                lr_for_variant = resolve_learning_rate(model_variant)
                optimizer  = torch.optim.AdamW(model.parameters(), lr=lr_for_variant)
                # Scheduler is per-head: the MLP head trains at a fixed LR
                # (USE_LR_SCHEDULER_MLP=False) while the LSTM head keeps
                # ReduceLROnPlateau.  train_gcn_*model tolerates scheduler=None.
                use_scheduler = (USE_LR_SCHEDULER_LSTM if _head == 'lstm'
                                 else USE_LR_SCHEDULER_MLP)
                if use_scheduler:
                ...
                # Leak-safe per-step graph rebuild context for the GCN/GAT heads.
                # Mirrors the Graph2Vec dynamic rebuild: at each step the ego-graph
                # is rebuilt from window [d-W, d-1] with the target row = the
                # model's own rolling predictions and every neighbour observed.
                # None under ablation or for the graph2vec heads (which rebuild
                # via recursive_forecast_graph2vec_perstep instead).
                gcn_rebuild_ctx = None
                if not ablate_z and not is_graph2vec and initial_target_window is not None:
                    gcn_rebuild_ctx = dict(
                        ...
                        enable_second_degree=enable_second_degree,
                        node_feature_mode=node_feature_mode,
                        nx_to_pyg=(nx_window_to_pyg_lstm
                                   if _variant_head(model_variant) == 'lstm'
                                   else nx_window_to_pyg_mlp),
                    )

                # Per-step neighbourhood log (n_neighbours / ids per forecast
                # step).  Populated by the rebuild path of every graph variant
                # (GCN/GAT/Graph2Vec); stays empty for ablation (no graph branch).
                _step_nbr_log: list = []
                ...
                if is_graph2vec:
                    ...
                    forecast = recursive_forecast_graph2vec_perstep(
                        ...
                        neighbour_log_out=_step_nbr_log,
                    )
                ...
                # ── Per-step neighbourhood size (graph variants only) ────────
                # _step_nbr_log is empty for ablation (no graph branch); only the
                # full GCN/GAT/Graph2Vec runs populate it.  Counts feed the
                # dedicated plot_results panel; ids feed the per-step hover.
                if _step_nbr_log:
                    _sorted_log = sorted(_step_nbr_log, key=lambda e: e['step'])
                    results_by_w_s[key]['neighbour_counts'][label_name] = [
                        e['n_neighbours'] for e in _sorted_log
                    ]
                    # Hover (keyed by threshold, variant-agnostic): last graph
                    # variant for this threshold wins — they share the topology.
                    if current_threshold is not None:
                        _all_step_neighbours[current_threshold] = {
                            e['step']: e['neighbours'] for e in _sorted_log
                        }
            ...
            if SAVE_PLOTS:
                for (w, s), src in grouped_results.items():
                    ...
                    plot_results(
                        ...
                        neighbour_counts=merged.get('neighbour_counts'),
                    )
