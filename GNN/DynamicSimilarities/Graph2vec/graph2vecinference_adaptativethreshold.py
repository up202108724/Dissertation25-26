...
from GNN.DynamicSimilarities.GraphAnalysis.utils import compute_similarities_1vsAll, compute_distances_1vsAll, calculate_thresholds

def build_dynamic_graph_with_adaptativethreshold(target_id, target_preds, df_wide, cat_labels, date_cols, metric, historical_weights=None, adaptive_strategy=None, percentile=None, enable_edges_within_star=True, enable_second_degree=False):
    G = nx.Graph()
    ...
    if percentile is not None:
        if len(valid_scores) > 0:
            if is_distance:
                ...
                pct_threshold = np.percentile(valid_scores, percentile)
                mask_pct = valid_scores <= pct_threshold
            else:
                # Highest similarities = most similar (e.g. top 5% -> 95th percentile)
                pct_threshold = np.percentile(valid_scores, 100 - percentile)
                mask_pct = valid_scores >= pct_threshold
        else:
            mask_pct = np.zeros_like(valid_scores, dtype=bool)
    else:
        mask_pct = np.ones_like(valid_scores, dtype=bool)
        
    scores_after_pct = valid_scores[mask_pct]
    
    if adaptive_strategy is not None and historical_weights is not None:
        # Add new window's valid score weights (that passed the percentile)
        updated_weights = historical_weights + scores_after_pct.tolist()
        
        # Calculate thresholds on the combined history
        metric_type = "distance" if is_distance else "similarity"
        candidate_thresholds = calculate_thresholds(updated_weights, metric_type, k_std=1.5, p_cutoff=90)
        
        # Select our specific strategy
        dynamic_threshold = candidate_thresholds.get(adaptive_strategy, float('inf') if is_distance else -float('inf'))
    else:
        dynamic_threshold = float('inf') if is_distance else -float('inf')
        
    if is_distance:
        mask_adaptive = valid_scores <= dynamic_threshold
    else:
        mask_adaptive = valid_scores >= dynamic_threshold
        
    # Combine the masks - must pass both percentile (local) and adaptive threshold (global)
    final_mask = mask_pct & mask_adaptive
    ...
    if enable_edges_within_star and len(neighbor_indices) > 1:
        ...
        for i, n_id1 in enumerate(n_ids):
            ...
            for j in range(i + 1, len(n_ids)):
                # Here we apply the same global dynamic threshold
    ...
    return G, updated_weights if (adaptive_strategy is not None and historical_weights is not None) else (historical_weights if historical_weights is not None else [])
...
def graph2vec_inference(
    ...
    historical_weights=None, adaptive_strategy=None
):
    ...
    current_historical_weights = historical_weights if historical_weights is not None else []
            ...
            for step in range(total_steps):
                ...
                if current_emb_seq is not None and step + 1 < total_steps:
                    ...
                    if start_date_idx >= 0 and end_date_idx <= len(df_wide_cols):
                        ...
                        new_G, current_historical_weights = build_dynamic_graph_with_adaptativethreshold(
                            target_id=item_id,
                            ...
                            metric=metric,
                            historical_weights=current_historical_weights,
