...
def build_statistical_similarity_graph(
    ...
    adj_list = {item: [] for item in item_ids}
    num_edges = 0
    ...
    if k is not None:
        ...
        for i in range(n):
            ...
            for neighbor_id, sim_value in top_k_neighbors.items():
                ...
                if use_absolute_similarity or sim_value >= similarity_threshold:
                    adj_list[item_ids[i]].append({'node': neighbor_id, 'weight': float(edge_weight), 'similarity': float(sim_value)})
                    num_edges += 1
                    '''
                    print(
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if condition:
                    adj_list[item_ids[i]].append({'node': item_ids[j], 'weight': float(edge_weight), 'similarity': float(sim_value)})
                    adj_list[item_ids[j]].append({'node': item_ids[i], 'weight': float(edge_weight), 'similarity': float(sim_value)})
                    num_edges += 1
                    '''
                    print(
                        f"Added edge between {item_ids[i]} and {item_ids[j]} "
...
def process_single_window(
    start_idx, unique_dates, df, date_col, item_col, target_col, window_size,
    aggfunc, similarity_method, similarity_threshold, k, use_absolute_similarity
):
    ...
    return graph_obj, sim_df, df_pivot, {"start_date": start_date, "end_date": end_date}
...
def build_dynamic_similarity_graphs(
    ...
    over the dates in the dataframe (parallelized over threads to keep GPU fed).
    """
    unique_dates = sorted(df[date_col].unique())
    num_dates = len(unique_dates)

    start_indices = list(range(0, num_dates - window_size + 1, step_size))
    print(f"Parallelizing {len(start_indices)} windows across {n_jobs} threads...")

    results = Parallel(n_jobs=n_jobs, backend="threading")(
        delayed(process_single_window)(
            start_idx, unique_dates, df, date_col, item_col, target_col, 
            window_size, aggfunc, similarity_method, similarity_threshold, 
            k, use_absolute_similarity
        )
        for start_idx in start_indices
    )
    
    # Unpack results preserving order
    graphs = [r[0] for r in results]
    sim_dfs = [r[1] for r in results]
    df_pivots = [r[2] for r in results]
    window_info = [r[3] for r in results]
