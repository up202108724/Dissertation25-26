...
def plot_distribution(all_values, metric, metric_type, window_size,
                      ...
                      percentile_markers=(50, 75, 90, 95, 99),
    ...
    tag = f"focal_vs_full_{metric}_{metric_type}_W{window_size}_S{step_size}"
    ...
    ax2    = ax.twinx()
    ax2.plot(x_kde, kde(x_kde), color='steelblue', linewidth=1.6, label='KDE')
    ...
    ax.set_title(
        f"Focal-vs-Full Pairwise {metric_type.capitalize()} Distribution — {metric} | "
    ...
    ax.set_title(f"Boxplot Focal-vs-Full ({metric}) | window={window_size} days")
    ax.set_xlabel(f"{metric_type.capitalize()} ({metric})", fontsize=11)
    plt.tight_layout()
