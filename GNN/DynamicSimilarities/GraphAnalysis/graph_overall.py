"""
Unified GLOBAL edge-weight distribution builder for the sliding-window
neighborhood-graph construction.

Supports three filtering modes (mirroring `neighbourhood_graph`):

    MODE = "none"        -> aggregate ALL candidate values (no filtering).
                            No star/within-star logic. This is the raw
                            "candidate pool" the thresholding mechanism sees.

    MODE = "threshold"   -> apply a FIXED user-supplied THRESHOLD per window.
                            Keep values that pass:
                                >= THRESHOLD for similarity
                                <= THRESHOLD for distance
                            Optional within-star edges.

    MODE = "percentile"  -> Phase 1: collect candidates across all windows.
                            Phase 2: derive a single global threshold matching
                            the top-k PERCENTILE and apply it per window.
                            Optional within-star edges.

Per item_id and per window:
  - compute the 1-vs-All metric vs every other active item,
  - filter according to MODE,
  - (optionally) extend with within-star pairwise edges between selected
    neighbors that also pass the same threshold.

Then aggregate the kept weights across all items/windows and plot the
distribution.

Configurable:
    - MODE        : 'none' | 'threshold' | 'percentile'
    - ITEM_IDS    : list of target products
    - METRIC      : metric name (e.g. 'spearman', 'pearson', 'kendall',
                                 'cid', 'dtw', 'manhattan', ...)
    - METRIC_TYPE : 'similarity' or 'distance'
    - WINDOW_SIZE : sliding window length
    - STEP_SIZE   : sliding step
    - THRESHOLD   : fixed cutoff value (MODE='threshold')
    - PERCENTILE  : top-k% to keep (MODE='percentile')
    - TRAIN_SIZE / VAL_SIZE : restrict to train+val portion of the dataset
    - ENABLE_EDGES_WITHIN_STAR : include pairwise edges between selected
                                 neighbors (ignored in MODE='none')
    - ENABLE_Z_NORMALIZATION   : per-item Z-score normalization (fit on
                                 train slice) — used for distance metrics
                                 so thresholds are comparable across items.
"""
