...
matches the GCN sibling so callers can reuse the GCNTimeSeriesDataset
+ collate_pyg_ts + recursive_forecast pipeline unchanged.
"""
...
class SimpleGATMLPForecaster(nn.Module):
    ...
    gcn_hidden       : int   – hidden width per head in GATConv₁
    ...
    gat_heads        : int   – number of attention heads in GATConv₁ (concat);
                              GATConv₂ always uses 1 head (mean)
    """
    ...
    def __init__(
        ...
        # ── GAT encoder ───────────────────────────────────────────────────
        # Layer 1: multi-head with concat → output dim = gcn_hidden * gat_heads
        self.conv1 = GATConv(in_channels, gcn_hidden, heads=gat_heads,
                             concat=True, add_self_loops=True, dropout=dropout)
        # Layer 2: single head with mean → output dim = d_g
        self.conv2 = GATConv(gcn_hidden * gat_heads, d_g, heads=1,
    ...
    def forward(self, pyg_batch, target_node_indices, ts_seq):
        ...
        if self.ablate_z:
            ...
            # Attention coefficients are learned from node features; edge_attr
            # is not passed so the graph topology alone guides aggregation.
