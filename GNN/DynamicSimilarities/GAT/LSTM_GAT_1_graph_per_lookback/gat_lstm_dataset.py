...
def _window_node_features(window_values: np.ndarray) -> np.ndarray:
    """
    window_values : (n_nodes, window_size) raw values per node over the window[cite: 1]
    Returns       : (n_nodes, 8) feature matrix[cite: 1]
                    [mean, std, min, max, first, last, slope, sum][cite: 1]
    """
