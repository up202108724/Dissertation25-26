...
class GATMLPForecaster(nn.Module):
    """
    GAT + MLP forecaster.

    A two-layer GATConv encoder processes the ego-graph. Edge weights
    (similarity/distance scores stored in edge_attr) are fed as edge features
    to bias the attention computation (edge_dim=1).  The target-node embedding
    z (shape B × out_channels) is concatenated with the flattened lookback
    sequence (ts_seq) and fed through a multi-layer MLP to produce horizon
    predictions.
    ...
    def __init__(
        ...
        hidden_channels: int,       # GATConv output dim per head (averaged over heads)
        out_channels: int,          # GATConv final output dim
        ...
        super().__init__()
        # Layer 1: multi-head attention, average over heads → hidden_channels
        self.conv1 = GATConv(in_channels, hidden_channels, heads=heads,
                             concat=False, dropout=dropout, edge_dim=1)
        # Layer 2: single-head → out_channels
        self.conv2 = GATConv(hidden_channels, out_channels, heads=1,
                             concat=False, dropout=dropout, edge_dim=1)
        self.activation = nn.ReLU()
        self.gat_drop = nn.Dropout(p=dropout)

        # MLP over [z || flat_ts_seq]
        mlp_in = out_channels + ts_input_size
        layers = []
        prev = mlp_in
    ...
    def forward(self, pyg_batch, target_node_indices, ts_seq):
        ...
        Returns : (B, horizon, 1)
        """
        # 1. GAT → target-node embedding z
        # GATConv uses edge_attr (E, edge_dim) to bias attention scores
        ea = pyg_batch.edge_attr
        ea = ea if (ea is not None and ea.shape[0] > 0) else None
        ...
        # 2. Flatten ts_seq and concatenate with z
        B = z.size(0)
        ts_flat = ts_seq.view(B, -1)            # (B, lookback * (1 + cal_dim))
        combined = torch.cat([z, ts_flat], dim=-1)  # (B, out_channels + ts_input_size)

        # 3. MLP → forecast
        pred = self.mlp(combined)               # (B, horizon)
