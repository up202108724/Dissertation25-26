...
from train import TrainConfig, train_gat_mlp
from gatinference import recursive_inference_gat_mlp
        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                for param_val, window_sz, step_sz, enable_edges, enable_second_degree in iterator:
                    ...
                    # ── GAT+MLP with embeddings ──────────────────────────────
                    print(f"\n{'='*60}")
                    param_str = f"threshold={current_threshold}" if is_threshold_mode else f"percentile={current_percentile}"
                    print(f"Running GAT+MLP: metric={metric}, {param_str}, window_size={window_sz}, enable_edges={enable_edges}, 2nd_degree={enable_second_degree}")
                    print(f"{'='*60}")
                    ...
                    model, _, t_losses, v_losses, best_epoch = train_gat_mlp(
                    ...
                    forecast = recursive_inference_gat_mlp(
                        model=model,
                        scaler=scaler,
