...
class GATLSTMForecaster(nn.Module):
    """
    GAT encoder + LSTM temporal model.

    The GAT encoder processes the ego-graph and produces a target-node
    embedding z (shape B × gat_out_ch).  z is projected to initialise both
    ...
    def __init__(
        ...
        super().__init__()
        # Layer 1: multi-head attention (concat=True → hidden_channels * gat_heads)
        self.conv1 = GATConv(in_channels, hidden_channels, heads=gat_heads,
                             concat=True, dropout=dropout)
        # Layer 2: single head (concat=False → out_channels)
        self.conv2 = GATConv(hidden_channels * gat_heads, out_channels, heads=1,
