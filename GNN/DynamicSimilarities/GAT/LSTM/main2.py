"""
main2.py  –  Grid search over spearman thresholds for GNN+LSTM only.

Thresholds: 0.82, 0.85, 0.88
Models run: GAT+LSTM (train_gat_lstm / recursive_inference_gat_lstm)
Results saved to: gnn_lstm_spearman_grid.csv
"""
...
from train import TrainConfig, train_gat_lstm
from gatinference import recursive_inference_gat_lstm
...
# ── CSV output ────────────────────────────────────────────────────────────────
CSV_PATH = os.path.join(script_dir, "gnn_lstm_spearman_grid.csv")
CSV_COLS = [
...
def _compute_metrics(actual, forecast):
    ...
    score = 0.50 * rmse + 0.25 * mae + 0.25 * abs(bias)
...
def main():
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        plots_dir = os.path.join(script_dir, 'gnn_lstm_spearman_plots')
        ...
        for seed in SEEDS:
            ...
            for threshold, window_sz in itertools.product(SPEARMAN_THRESHOLDS, WINDOW_SIZES):
                ...
                t0 = time.time()
                model, trained_scaler, t_losses, v_losses, best_epoch = train_gat_lstm(
                    df=df_product_lstm,
                    cfg=cfg,
                    seed=seed,
                    loss_type='mse',
                ...
                forecast = recursive_inference_gat_lstm(
                    model=model,
            ...
            if forecasts_dict:
                ...
                plot_results(
                    ...
                    target_col=TARGET_COL,
                    title=f'GAT+LSTM Spearman Threshold Comparison | Item {product_id} | Store {store_id}',
                    save_path=plot_save,
