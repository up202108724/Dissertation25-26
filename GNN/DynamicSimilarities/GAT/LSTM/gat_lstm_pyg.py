...
class GATEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout=0.2, heads=4):
        super(GATEncoder, self).__init__()
        self.conv1 = GATConv(in_channels, hidden_channels, heads=heads, concat=True, edge_dim=1, dropout=dropout)
        self.conv2 = GATConv(hidden_channels * heads, out_channels, heads=1, concat=False, edge_dim=1, dropout=dropout)
...
def generate_node_features(ts, cal_next=None, cal_lookback=None, selected_features=None,
    ...
    NODE_FEATURES list can be shared between the GraphSAGE and GAT models.
...
class GATLSTMForecaster(nn.Module):
    """
    GAT encoder + LSTM forecaster.

    Mirrors GraphSAGELSTMForecaster (graphsage_pyg.py) with GATConv instead of SAGEConv.
