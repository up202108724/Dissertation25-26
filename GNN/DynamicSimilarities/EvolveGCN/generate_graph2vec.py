...
def load_or_generate_embeddings(product_id, metric, window_size, step_size, threshold, percentile, dimensions=20, wl_iterations=2, walk_length=10, num_walks=50, workers=1, epochs=100, min_count=1, window=0, seed=42, use_residuals=False, model_type='ridge', enable_edges_within_star=True, enable_second_degree=False, run_id=None, overwrite_embeddings=False):
    ...
    if threshold is not None:
        ...
        emb_pkl_path = os.path.join(emb_base_dir, f"{prefix}embeddings_{metric}_Window{window_size}_Step{step_size}_th{threshold}_dim{dimensions}_ep{epochs}_wl{wl_iterations}_seed{seed}{run_suffix}.pkl")
    if percentile is not None:
        if use_residuals:
            ...
            emb_pkl_path = os.path.join(emb_base_dir, f"{prefix}embeddings_{metric}_Window{window_size}_Step{step_size}_top{percentile}pct_dim{dimensions}_ep{epochs}_wl{wl_iterations}_seed{seed}{run_suffix}.pkl")
        else:
            pkl_path = os.path.join(base_dir, f"{prefix}dynamic_graphs_{metric}_Window{window_size}_Step{step_size}_pct{percentile}.pkl")
            emb_pkl_path = os.path.join(emb_base_dir, f"{prefix}embeddings_{metric}_Window{window_size}_Step{step_size}_pct{percentile}_dim{dimensions}_ep{epochs}_wl{wl_iterations}_seed{seed}{run_suffix}.pkl")
