...
class GCNTimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # graph aligned to the LAST step of the LSTM input window
        g_idx = idx + self.seq_length - 1
        g_idx = min(max(g_idx, 0), len(self.pyg_graphs) - 1)
        graph = self.pyg_graphs[g_idx]

        ts_tensor = torch.from_numpy(ts.astype(np.float32))
        y_tensor  = torch.tensor([y], dtype=torch.float32)
        return graph, ts_tensor, y_tensor


# ── collate ────────────────────────────────────────────────────────────────
def collate_pyg_ts(batch):
    """
    Batches a list of (Data, ts_seq, y) tuples into:
        pyg_batch  : torch_geometric.data.Batch
        ts_batch   : (B, L, F)
        y_batch    : (B, 1)
        target_idx : (B,)   indices of each graph's target node within the batch
                            — convenient pointer for SimpleGCNLSTMForecaster.
    """
    graphs, ts_seqs, ys = zip(*batch)
    pyg_batch  = Batch.from_data_list(list(graphs))
    ts_batch   = torch.stack(ts_seqs, dim=0)
    y_batch    = torch.stack(ys, dim=0)
    # target node = first node of each subgraph (ego-graph convention).
    target_idx = pyg_batch.ptr[:-1]
    return pyg_batch, ts_batch, y_batch, target_idx
