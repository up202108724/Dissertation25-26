...
class GCN_LSTM_InitState(nn.Module):
    def __init__(
        ...
        self.h0_proj = nn.Linear(gcn_embed_dim, lstm_hidden_size)
        self.c0_proj = nn.Linear(gcn_embed_dim, lstm_hidden_size)
        self.fc = nn.Linear(lstm_hidden_size, horizon)

    def forward(self, ts_x, graph_x, graph_adj, target_node_idx):
        # Graph branch (Compute embeddings end-to-end)
        h_gcn = F.relu(self.gcn.gc1(graph_x, graph_adj))
        node_embeddings = self.gcn.gc2(h_gcn, graph_adj)   # (N, Dg)
        ...
        # Project Graph Embeddings to initial LSTM Hidden and Cell states
