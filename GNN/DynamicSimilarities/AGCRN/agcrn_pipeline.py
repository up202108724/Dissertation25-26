"""
AGCRN end-to-end pipeline for the 61-product, single-store benchmark.

Trains ONE global AGCRN over all 61 products jointly and evaluates it
...
from agcrn_model import AGCRN                                # noqa: E402
...
PLOTS_DIR     = os.path.join(SCRIPT_DIR, "plots_agcrn")
...
def evaluate_and_plot(preds, data, train_losses, val_losses):
    ...
    for n, iid in enumerate(data["items"]):
        ...
        plot_results(
            ...
            title=f"AGCRN — item {iid}",
            save_path=os.path.join(PLOTS_DIR, f"agcrn_item_{iid}.png"),
...
def main():
    ...
    model = AGCRN(num_nodes=data["N"], in_dim=X.shape[-1], out_dim=1,
                  horizon=1, rnn_units=RNN_UNITS, embed_dim=EMBED_DIM,
                  num_layers=NUM_LAYERS, cheb_k=CHEB_K).to(device)
    print(f"  AGCRN params: {sum(p.numel() for p in model.parameters()):,}")
    ...
    out_csv = os.path.join(SCRIPT_DIR, "agcrn_results.csv")
    res.to_csv(out_csv, index=False)
    np.save(os.path.join(SCRIPT_DIR, "agcrn_learned_adjacency.npy"),
