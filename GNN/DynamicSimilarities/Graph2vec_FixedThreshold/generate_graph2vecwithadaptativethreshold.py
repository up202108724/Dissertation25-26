...
def load_or_generate_embeddings(product_id, metric, window_size, step_size, threshold, percentile, 
        ...
        overwrite_embeddings=False, calculated_threshold_strategy=None, graphs=None, df=None, metric_type=None, cat_labels=None, train_end_idx=None, strategy_args=None
            ):
    ...
    if calculated_threshold_strategy is not None:
        import re
        safe_strat_name = re.sub(r'[^a-zA-Z0-9_\-]', '', calculated_threshold_strategy.replace(' ', '_'))
        plot_dir_name = os.path.join(plot_dir_name, safe_strat_name)
        ...
        if graphs is None:
            if os.path.exists(pkl_path):
                ...
                graphs, fixed_global_threshold = neighbourhood_graph(
                    ...
                    calculated_threshold_strategy=calculated_threshold_strategy,
