...
def recursive_inference(
    ...
    if graph2vec_model is not None and recent_embeddings is not None:
        ...
        current_emb_window = recent_embeddings.copy().astype(np.float32)  # (L, emb_dim)
    else:
        current_emb_window = np.zeros((current_x_scaled.shape[0], 0), dtype=np.float32)
    ...
    with torch.no_grad(): # BOA PRÁTICA: Desligar gradientes na inferência
        for i in range(horizon):
            ...
            emb_tensor = torch.from_numpy(current_emb_window).float().unsqueeze(0).to(device) # (1, L, emb_dim)

            y_pred = model(x_tensor, emb_tensor)
            ...
            # 3. Shift do embedding window e inferir embedding para o novo step
            current_emb_window = np.roll(current_emb_window, -1, axis=0)
