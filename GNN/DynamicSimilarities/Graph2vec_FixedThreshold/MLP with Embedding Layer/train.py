...
import pandas as pd
import os
from mlp import MLPForecaster, MLPWithLearnedEmbedding
...
def train_mlp_forecaster(
    ...
    # Create windows — embeddings are returned as a separate array, NOT stacked into X
    X_train, emb_train_w, y_train_full = make_windows(
        train_scaled, cfg.lookback, cfg.horizon, target_channel=target_channel, embeddings=emb_train, graph_window_size=15
    )
    X_val, emb_val_w, y_val_full = make_windows(
    ...
    # in_channels is target + exog only (no embedding columns)
    C_in = X_train.shape[2]
    emb_dim = emb_train_w.shape[2]
    ...
    train_loader = DataLoader(WindowDataset(X_train, emb_train_w, y_train), batch_size=cfg.batch_size, shuffle=True)
    val_loader = DataLoader(WindowDataset(X_val, emb_val_w, y_val), batch_size=cfg.batch_size, shuffle=False)
    ...
    for epoch in range(1, cfg.epochs + 1):
        model.train()
        train_loss = 0.0
        for xb, emb_b, yb in train_loader:
            xb   = xb.to(cfg.device)
