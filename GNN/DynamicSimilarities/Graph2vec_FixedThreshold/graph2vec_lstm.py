...
from GNN.DynamicSimilarities.Graph2vec.generate_graph2vecwithadaptativethreshold import load_or_generate_embeddings, infer_metric_type
...
adaptative_strategies = [
    (None, {})
]

for k in k_stds:
    adaptative_strategies.append(('Mean - k*Std', {'k_std': k}))
    adaptative_strategies.append(('Median - k*MAD', {'k_std': k}))
    adaptative_strategies.append(('CV', {'k_std': k}))

for k in k_iqrs:
    adaptative_strategies.append(('IQR', {'k_iqr': k}))

for p in p_cutoffs:
    adaptative_strategies.append(('Percentile', {'p_cutoff': p}))

adaptative_strategies.append(('Knee', {}))

def safe_adaptive_strategy_name(adaptive_strategy, strategy_args=None):
    if adaptive_strategy is None:
        return None
    base_name = re.sub(r'[^a-zA-Z0-9_\-]', '', str(adaptive_strategy).replace(' ', '_'))
    if strategy_args:
        for key, val in sorted(strategy_args.items()):
            base_name += f"_{key}_{val}"
    return base_name
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        for seed in SEEDS:
            ...
            for metric in all_metrics:
                ...
                if metric == 'no_emb':
                    iterator = [(None, 15, 1, False, False, (None, None))]
                else:
                    iterator = itertools.product(percentiles, window_sizes, step_sizes, enable_edges_opts, enable_second_degree_opts, adaptative_strategies)
            
                for percentile, window_size, step_size, enable_edges, enable_second_degree, (adaptive_strat, p_ctf) in iterator:
                    use_embeddings = (metric != 'no_emb')

                    key = (window_size, step_size)
                    ...
                    print(f"\n{'='*60}")
                    if use_embeddings:
                        print(f"Running Experiment: metric={metric}, percentile={percentile}, window_size={window_size}, enable_edges={enable_edges}, 2nd_degree={enable_second_degree}, strategy={adaptive_strat}, p_cutoff={p_ctf}")
                    else:
                    ...
                    if use_embeddings:
                        ...
                        graph_embeddings, graph2vec_model, csv_path, build_time, emb_time, fixed_threshold = load_or_generate_embeddings(
                            ...
                            seed=seed,
                            calculated_threshold_strategy=adaptive_strat,
                            train_end_idx=val_start_idx,
                            df=df_wide_global,
                            cat_labels=cat_labels_dict,
                            strategy_args={'p_cutoff': p_ctf if p_ctf is not None else 5}
                        )
                        print(f"Resolved graph threshold for strategy={adaptive_strat}: {fixed_threshold}")
                    ...
                    if use_embeddings:
                        if csv_path:
                            ...
                            strategy_name = safe_adaptive_strategy_name(adaptive_strat, p_ctf)
                            if strategy_name:
                                base_name = base_name.replace('.pth', f'_strat_{strategy_name}.pth')
                            ...
                            strat_str = safe_adaptive_strategy_name(adaptive_strat, p_ctf)
                            ...
                            strategy_part = f'_strat_{strat_str}' if strat_str else ''
                            best_model_path = os.path.join(best_models_dir, f'{prefix}_{window_size}_{step_size}_pct_{percentile}{strategy_part}_seed_{seed}.pth')
                            history_path = os.path.join(best_models_dir, f'{prefix}_{window_size}_{step_size}_pct_{percentile}{strategy_part}_seed_{seed}_history.pkl')
                    else:
                    ...
                    strat_str = str(adaptive_strat).replace(' ', '')
                    if p_ctf is not None and adaptive_strat == 'Percentile':
                        strat_str += f"_{p_ctf}"
                    label_name = f"pct:{percentile}|w:{window_size}|st:{step_size}|e:{enable_edges}|2nd:{enable_second_degree}|st:{strat_str}" if use_embeddings else "No Embeddings"
                ...
                if metric == 'no_emb':
                    ...
                    plot_results(train, val, test, results_by_w_s[(15, 1)]['forecasts'], train_index, val_index, test_index,
                                 results_by_w_s[(15, 1)]['train_losses'], results_by_w_s[(15, 1)]['val_losses'], metric=metric, embedding_strategy='graph2vec',
                                 target_col=TARGET_COL, title=f'{emb_title} (Item={product_id})', seed=seed,
                                 save_path=save_plot_path, rmse=results_by_w_s[(15, 1)]['rmse'], mae=results_by_w_s[(15, 1)]['mae'], 
                                 bias=results_by_w_s[(15, 1)]['bias'], score=results_by_w_s[(15, 1)]['score'], pocid=results_by_w_s[(15, 1)]['pocid'])
                else:
                    metric_type = infer_metric_type(metric)
                    for (w, s), res_dicts in results_by_w_s.items():
                        sub_dir = os.path.join(grid_search_plots_dir, metric_type, f'window_{w}', f'step_{s}', f'item_{product_id}')
                        os.makedirs(sub_dir, exist_ok=True)
                        save_plot_path = os.path.join(sub_dir, f"item_{product_id}_store_{store_id}_{metric}_seed_{seed}.html")
