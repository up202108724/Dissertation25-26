...
def nx_window_to_pyg(
    ...
    node_feature_mode : 'raw' (full window sequence, width = window_size) or
...
# ── seq-target dataset (time-distributed supervision) ───────────────────────
class GCNSeqTargetDataset(Dataset):
    """
    Like ``GCNTimeSeriesDataset`` but returns per-step targets so the
    time-distributed head can be supervised at every lookback step.

    __getitem__ returns ``(graphs, ts_seq, y_seq)`` where
        y_seq : FloatTensor (seq_length, 1)
                targets[idx+1 .. idx+seq_length] — position k predicts k+1.

    The seq-to-one head can still be trained with this dataset by reading
    only ``y_seq[:, -1, :]`` inside the training loop.
    """
    ...
    def __getitem__(self, idx):
        target_seq = self.target_data[idx : idx + self.seq_length]
        # position k predicts the next value → aligned with exog at k
        ...
        ts_tensor = torch.from_numpy(ts.astype(np.float32))
        y_tensor  = torch.from_numpy(y_seq.reshape(-1, 1).astype(np.float32))
        return graphs, ts_tensor, y_tensor
...
def collate_pyg_ts_seq(batch):
    """
    Collate for ``GCNSeqTargetDataset``.  Same as ``collate_pyg_ts`` except
    ``y_batch`` is ``(B, L, 1)`` instead of ``(B, 1)``, carrying per-step
    targets for the time-distributed head.  The seq-to-one head should
    index ``y_batch[:, -1, :]`` to obtain the single-step target.
        ...
        L          : int
    """
    graph_lists, ts_seqs, y_seqs = zip(*batch)
    L = len(graph_lists[0])
    flat_graphs = [g for gl in graph_lists for g in gl]
    pyg_batch   = Batch.from_data_list(flat_graphs)
    ts_batch    = torch.stack(ts_seqs, dim=0)    # (B, L, F)
    y_batch     = torch.stack(y_seqs, dim=0)     # (B, L, 1)
    target_idx  = pyg_batch.ptr[:-1]             # (B*L,)
    return pyg_batch, ts_batch, y_batch, target_idx, L
