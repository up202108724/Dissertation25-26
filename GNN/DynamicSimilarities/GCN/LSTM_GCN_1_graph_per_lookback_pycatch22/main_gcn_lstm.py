...
# Number of independent runs (random model init) per product.
# Since no global seed is set, each run uses a different random initialisation.
N_EXPERIENCES  = 5
...
def main():
    ...
    product_times = []          # (product_id, store_id, exp_idx, seconds)
    total_t0 = time.time()

    for (product_id, store_id), exp_idx in itertools.product(PRODUCTS_TO_TEST, range(N_EXPERIENCES)):
        print(f"\n{'='*80}")
        print(f"PROCESSING PRODUCT {product_id} FOR STORE {store_id}  [Experience {exp_idx + 1}/{N_EXPERIENCES}]")
        print(f"{'='*80}\n")
        ...
        if RUN_LSTM_BASELINE:
            ...
            bl_model_path = os.path.join(
                ...
                f"lstm_baseline_{product_id}_{store_id}_exp{exp_idx}.pth",
            )
        ...
        for config in all_configs:
            ...
            for ablate_z, param_val, window_size, step_size, enable_edges, enable_second_degree in iterator:
                ...
                base_name = (
                    ...
                    f"_w{window_size}_s{step_size}_{param_label}{res_tag}_exp{exp_idx}"
                )
                ...
                if os.path.exists(best_model_path) and os.path.exists(history_path):
                    ...
                    diag_csv_path = os.path.join(
                        product_diag_dir,
                        DIAG_CSV_NAME.replace(".csv", f"{diag_suffix}_exp{exp_idx}.csv"),
                    )
                ...
                with open(csv_results_path, 'a', newline='') as csvfile:
                    ...
                    if not file_exists:
                        writer.writerow([
                            "product_id", "store_id", "exp_idx", "metric",
            ...
            if SAVE_PLOTS:
                ...
                for (w, s) in ws_combos:
                    ...
                    sub_dir = os.path.join(
                        ...
                        f'item_{product_id}', values_str, f'exp_{exp_idx}',
                    )
        ...
        product_times.append((product_id, store_id, exp_idx, product_elapsed))
        print(f"\n[TIMING] Product {product_id} | store {store_id} | exp {exp_idx}: "
    ...
    with open(timings_csv_path, "w", newline="") as fh:
        ...
        w.writerow(["product_id", "store_id", "exp_idx", "seconds"])
