...
from torch_geometric.data import Batch, Data
                                   # optional dep, only needed for catch22 modes
import pycatch22 
...
_EXTRA_STATS_WIDTH = 3                 # max + last + zero_fraction (see below)


# ── node feature builder ───────────────────────────────────────────────────
def _window_node_features_catch22(
    window_values: np.ndarray, catch24: bool = True, extra_stats: bool = True
) -> np.ndarray:
    """
    ...
    extra_stats   : when True append 3 further scale/level features that
                    catch22's z-normalisation also throws away and that matter
                    for erratic retail demand:
                      • max            — peak demand / spikes (not in mean±std)
                      • last           — current level, the strongest forecast cue
                      • zero_fraction  — share of zero days, i.e. intermittency
                    (min and sum are deliberately omitted: min ≈ 0 and collinear
                    with zero_fraction, sum ≈ mean × window_size is recoverable
                    from DN_Mean at fixed window length.)
    Returns       : (n_nodes, W) where W = (24 if catch24 else 22)
                    + (3 if extra_stats else 0)  →  e.g. 27 for catch24+extras.
    ...
    catch24 / extra_stats is used.  Short or flat windows (e.g. a 30-day run of
    zeros — common in erratic retail data) make several features ill-defined and
    return NaN; those are replaced with 0.0 so a single degenerate node cannot
    poison the whole graph during GCN message passing.
    """
    ...
    catch_width = _CATCH24_WIDTH if catch24 else _CATCH22_WIDTH
    width = catch_width + (_EXTRA_STATS_WIDTH if extra_stats else 0)
    ...
    for i in range(n_nodes):
        ...
        try:
            ...
            feats[i, :catch_width] = np.asarray(vals, dtype=np.float32)
        except Exception:
            # flat / constant series can crash the C backend → leave zeros
            feats[i, :catch_width] = 0.0
    if extra_stats:
        # Scale/level features that catch22 z-normalisation discards.
        mx        = window_values.max(axis=1)
        last      = window_values[:, -1]
        zero_frac = (window_values == 0).mean(axis=1)
        feats[:, catch_width:] = np.stack([mx, last, zero_frac], axis=1).astype(np.float32)
...
def build_pyg_graphs_from_nx_windows(
    ...
    for i, G in enumerate(graphs):
        ...
        pyg_list.append(nx_window_to_pyg(
...
def nx_window_to_pyg(
    ...
    node_feature_mode : 'raw'      full window sequence, width = window_size
                        'stats'    8-dim summary [mean, std, min, max, first,
                                   last, slope, sum]
                        'catch22'  22 catch22 shape/dynamics features
                        'catch24'  22 catch22 + DN_Mean + DN_Spread_Std (24-d)
    """
    ...
    if node_feature_mode == 'raw':
        x = torch.from_numpy(_window_node_features_raw_values(window_values))
        ...
        x = torch.from_numpy(_window_node_features_catch22(
            window_values, catch24=(node_feature_mode == 'catch24'),extra_stats=True))
    else:
        raise ValueError(
...
# ── per-column node-feature standardisation ─────────────────────────────────
def fit_node_feature_scaler(graphs):
    """
    Fit a per-column ``StandardScaler`` on the node-feature rows of ``graphs``
    (pass the TRAINING graphs only — never val/test — to avoid leakage).

    Why: catch22 z-normalises each series internally, so its 22/24 features sit
    in roughly [-3, 3], whereas the ``extra_stats`` columns (max / last) are on
    the raw demand scale.  Without standardisation those raw-scale columns
    dominate GCN message passing (neighbour averaging is skewed by a single
    high-``max`` node).  Standardising per column puts every feature on the same
    footing so ``extra_stats`` is tested on its merits rather than its magnitude.

    All-zero rows — produced by zero-pad graphs and by flat windows whose
    catch22 backend returned zeros — are excluded so they don't bias the column
    mean/std.  Returns a fitted ``StandardScaler``, or ``None`` if no usable rows.
    """
    from sklearn.preprocessing import StandardScaler

    rows = [g.x.detach().cpu().numpy() for g in graphs]
    if not rows:
        return None
    X = np.concatenate(rows, axis=0)
    X = X[~np.all(X == 0.0, axis=1)]            # drop pad / degenerate rows
    if X.shape[0] == 0:
        return None
...
def apply_node_feature_scaler(graphs, scaler) -> None:
    """
    In-place per-column standardisation of every graph's ``x`` using a scaler
    from :func:`fit_node_feature_scaler`.

    Pure pad graphs (all-zero feature matrix) are left untouched so they remain
    neutral placeholders.  Graphs are de-duplicated by object id because the
    per-day timeline (see ``_align_pyg_windows_to_timeline`` in the driver)
    aliases the *same* ``Data`` object across padded and repeated days — without
    de-duplication a shared object would be transformed multiple times.
    """
    if scaler is None:
        return
    seen = set()
    for g in graphs:
        ...
        if np.all(x == 0.0):                    # leave pad graphs neutral
            continue
        g.x = torch.from_numpy(scaler.transform(x).astype(np.float32))
