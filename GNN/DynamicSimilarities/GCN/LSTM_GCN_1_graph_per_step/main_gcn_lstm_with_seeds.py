...
# Per-column standardisation of node features, fit on TRAIN graphs only and
# applied across the whole timeline before the GCN.  Keeps the raw-scale
# extra_stats columns (max/last) from dominating the z-normalised catch22
# block during message passing.  Toggle to compare with/without.
...
def _node_feature_width(mode, window_size):
    ...
    return {'stats': 8, 'raw': window_size,
            'catch22': 22, 'catch24': 24}[mode]
...
def main():
    ...
    for (product_id, store_id), seed in itertools.product(PRODUCTS_TO_TEST, SEEDS):
        ...
        for config in all_configs:
            ...
            for ablate_z, param_val, window_size, step_size, enable_edges, enable_second_degree, node_feature_mode in iterator:
                ...
                if ablate_z:
                    ...
                    node_feat_scaler  = None     # GCN zeroed in ablation
                else:
                    ...
                    # Per-column standardisation: fit on TRAIN graphs only, then
                    # apply across the whole timeline.  train/val/seed/future are
                    # slices of pyg_aligned_global, so they share these Data
                    # objects and get standardised in place too.
                    node_feat_scaler = None
                    if STANDARDIZE_NODE_FEATURES:
                        node_feat_scaler = fit_node_feature_scaler(pyg_train)
                        apply_node_feature_scaler(pyg_aligned_global, node_feat_scaler)
                ...
                if os.path.exists(best_model_path) and os.path.exists(history_path):
                    ...
                    diag_csv_path = os.path.join(
                        product_diag_dir,
                        DIAG_CSV_NAME.replace(".csv", f"{diag_suffix}_exp{seed}.csv"),
                    )
                ...
                # so that _recursive_forecast_gcn_perstep can patch future_graphs[i].x[0]
                ...
                forecast = _recursive_forecast_gcn_perstep(
                    ...
                    node_feature_mode=node_feature_mode,
                    node_feature_scaler=node_feat_scaler,
                )
            ...
            grouped_results = {}
            for (ablate_z_key, _, w, s), res_dicts in results_by_w_s.items():
