    ...
    for step in range(horizon):
        ...
        if target_window is not None:
            ...
            if node_feature_mode == 'raw':
                ...
                new_feats = _window_node_features_catch22(
                    target_window[None, :], catch24=(node_feature_mode == 'catch24'), extra_stats=True)
            else:
                new_feats = _window_node_features(target_window[None, :])
            # Standardise with the SAME per-column scaler used on the training
            # graphs, so the rebuilt target row matches the scale the model saw.
            if node_feature_scaler is not None:
                new_feats = node_feature_scaler.transform(new_feats).astype(np.float32)
