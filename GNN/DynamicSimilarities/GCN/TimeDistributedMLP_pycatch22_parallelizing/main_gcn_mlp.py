...
# ──────────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="GCN+MLP per-step grid-search runner")
    parser.add_argument("--seed", type=int, default=None,
                        help="Run a single seed (overrides SEEDS list).")
    parser.add_argument("--worker-id", type=int, default=0,
                        help="0-based worker index for product sharding.")
    parser.add_argument("--num-workers", type=int, default=1,
                        help="Total number of parallel workers.")
    args = parser.parse_args()

    # Override seed list if a specific seed was requested
    global SEEDS
