...
try:                                   # optional dep, only needed for catch22 modes
...
def _window_node_features_catch22(
    ...
    catch24 is used.  Short or flat windows (e.g. a 30-day run of zeros) make
    several features ill-defined and return NaN; those are replaced with 0.0 so
    a single degenerate node cannot poison the whole graph during GCN message
    passing.
    """
    if window_values.ndim != 2:
        raise ValueError("window_values must be 2D (n_nodes, window_size)")
    if _pycatch22 is None:
        raise ImportError(
            "pycatch22 is not installed but node_feature_mode requires it "
    ...
    for i in range(n_nodes):
        ...
        try:
            vals = _pycatch22.catch22_all(ts, catch24=catch24)["values"]
...
def nx_window_to_pyg(
    ...
    if node_feature_mode == 'stats':
        ...
        x = torch.from_numpy(_window_node_features_catch22(
            window_values, catch24=(node_feature_mode == 'catch24')))
    else:
        raise ValueError(
