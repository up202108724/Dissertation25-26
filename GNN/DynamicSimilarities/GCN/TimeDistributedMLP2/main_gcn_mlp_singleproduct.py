...
grid_configs = [
    ...
    {'metric': 'spearman', 'threshold': 0.82},
]
    ...
    for product_id, store_id in PRODUCTS_TO_TEST:
        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                # Single-threshold mode: read the scalar 'threshold' key and wrap
                # it in a one-element list so the rest of the pipeline runs exactly
                # one experiment at that threshold.
                ...
                _all_step_neighbours: dict = {}        # step_idx -> list of neighbour IDs (all steps)
                ...
                for ablate_z, param_val, window_size, step_size, enable_edges, enable_second_degree in iterator:
                    ...
                    if _inf_nx_graphs is not None and not ablate_z and not _inference_step_dates:
                        ...
                        all_nbr_ids: set = set()
                        # Collect neighbours for ALL inference steps
                        for step_idx in range(len(_inf_nx_graphs)):
                            ...
                            # Red vline markers only at the selected highlight steps
