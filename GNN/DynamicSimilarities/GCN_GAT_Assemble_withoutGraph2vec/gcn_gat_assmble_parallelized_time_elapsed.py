...
def main():
    ...
    for (product_id, store_id), seed in itertools.product(PRODUCTS_TO_TEST, seeds_to_run):
        ...
        for config in all_configs:
            ...
            # ── Write baselines to CSV (once per metric, same columns) ────
            _baseline_csv_path = os.path.join(SCRIPT_DIR, f"{metric}{worker_csv_suffix}.csv")
            if RUN_LSTM_BASELINE and _bl_rmse is not None:
                with open(_baseline_csv_path, 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow([
                        product_id, store_id, seed, metric, 'lstm_baseline',
                        '', '', '', '', '', '', False, '',
                        _bl_rmse, _bl_mae, _bl_bias, _bl_score, _bl_pocid,
                    ])
            if RUN_MLP_BASELINE and _mlp_rmse is not None:
                with open(_baseline_csv_path, 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow([
                        product_id, store_id, seed, metric, 'mlp_baseline',
                        '', '', '', '', '', '', False, '',
                        _mlp_rmse, _mlp_mae, _mlp_bias, _mlp_score, _mlp_pocid,
                    ])
