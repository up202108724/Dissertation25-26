"""
Grid-search runner for the FIXED-GRAPH GCN + MLP forecaster.

One graph is built per product from the full train+val period (455 + 153 = 608 days).
train+val period (fixed topology + fixed node features).  The GCN encodes
this graph into a single embedding ``z`` that is broadcast across all L
lookback steps, concatenated with the temporal features, and fed into a flat
MLP head.

Reuses:
    * ``GCNFixedGraphDataset`` + ``collate_fixed_graph_ts`` +
      ``build_pyg_graphs_from_nx_windows`` from the local ``gcn_mlp_dataset``
    * ``_recursive_forecast_gcn_fixed`` from the local ``gcn_mlpinference``
...
# Reused graph-construction utilities
from gcn_mlp_dataset import (
    GCNFixedGraphDataset,
    collate_fixed_graph_ts,
    build_pyg_graphs_from_nx_windows,
)
from gcn_mlpinference import _recursive_forecast_gcn_fixed
...
grid_configs = [
    {'metric': 'spearman', 'thresholds': [0.6,0.65,0.7]},
]
        ...
        for seed in SEEDS:
            ...
            for config in all_configs:
                ...
                iterator = itertools.product(
                    ABLATE_Z_VALUES, params, SEQ_LENGTHS,
                    enable_edges_opts, enable_second_degree_opts,
                )

                metric_type = infer_metric_type(metric)

                for ablate_z, param_val, seq_length, enable_edges, enable_second_degree in iterator:
                    ...
                    key = (ablate_z, param_val, seq_length)
                    ...
                    print(f"Running Experiment: ablate_z={ablate_z}, metric={metric}, {param_str}, "
                          f"enable_edges={enable_edges}, 2nd_degree={enable_second_degree}")
                    print(f"{'='*60}")

                    # ── 1 & 2. Fixed graph (skipped for ablation) ───────────
                    train_val_size = train_size + val_size   # 455 + 153 = 608

                    if ablate_z:
                        # GCN output is zeroed; building the graph would be
                        # pure waste.  Use a single-node dummy graph instead.
                        fixed_threshold = None
                        results_by_w_s[key]['threshold'] = fixed_threshold
                        fixed_pyg = Data(
                            x=torch.zeros(1, train_val_size, dtype=torch.float32),
                            edge_index=torch.tensor([[0], [0]], dtype=torch.long),
                            edge_attr=torch.zeros(1, 1, dtype=torch.float32),
                            num_nodes=1,
                        )
                    else:
                        # Build ONE graph from the FULL train+val period.
                        # Slice df_wide_scaled to the product's train+val columns
                        # so neighbourhood_graph computes similarities from those
                        # 608 days only.  Node features = the full 608-day z-scaled
                        # sequence for each node in the ego-graph.
                        T_global = df_wide_scaled.shape[1]
                        product_offset = T_global - len(df_p)
                        tv_start = product_offset + train_start_idx
                        tv_end   = product_offset + test_start_idx    # = tv_start + 608
                        df_wide_train_val = df_wide_scaled.iloc[:, tv_start:tv_end]
                        ...
                        nx_graphs, fixed_threshold = neighbourhood_graph(
                            ...
                            df=df_wide_train_val,
                            metric=metric,
                            metric_type=metric_type,
                            window_size=train_val_size,   # one graph, full period
                            ...
                            train_end_idx=train_size,   # threshold from train only
                        )
                        print(f"Resolved graph threshold={current_threshold}: {fixed_threshold}")
                        results_by_w_s[key]['threshold'] = fixed_threshold

                        # Convert the single NX graph to a PyG Data object.
                        # Node features = the full 608-day window (in_channels = 608).
                        pyg_list = build_pyg_graphs_from_nx_windows(
                            nx_graphs, df_wide_train_val, product_id,
                            window_size=train_val_size, step_size=1,
                        )
                        fixed_pyg = pyg_list[0]

                    # ── 3. Datasets / loaders (FIXED-GRAPH) ──────────────────
                    use_pin_memory = torch.cuda.is_available()
                    train_dataset = GCNFixedGraphDataset(
                        target_data=train_scaled,
                        exog_data=exog_train_scaled if EXOG_COLS else None,
                        seq_length=seq_length,
                        fixed_pyg_graph=fixed_pyg,
                    )
                    train_loader = DataLoader(
                        train_dataset, batch_size=BATCH_SIZE, shuffle=False,
                        pin_memory=use_pin_memory, collate_fn=collate_fixed_graph_ts,
                    )
                    val_dataset = GCNFixedGraphDataset(
                    ...
                    in_channels   = fixed_pyg.x.shape[1]
                    ...
                    res_tag = "_res" if USE_RESIDUALS else ""
                    param_label = (f"th_{current_threshold}" if is_threshold_mode
                                   else f"pct_{current_percentile}")
                    base_name = (
                        f"best_gcnmlp_fixed_{prefix_star}{product_id}_{metric}"
                        f"_L{seq_length}_{param_label}{res_tag}_seed_{seed}"
                    )
                    ...
                    if os.path.exists(best_model_path) and os.path.exists(history_path):
                        ...
                        print("Training new fixed-graph GCN+MLP model...")
                    ...
                    forecast = _recursive_forecast_gcn_fixed(
                        model=model,
                        ts_seed=ts_seed,
                        fixed_graph=fixed_pyg,
                    ...
                    label_name = (f"{param_str_label}|L:{seq_length}"
                    ...
                    timing_csv_path = os.path.join(SCRIPT_DIR, "timing.csv")
                    timing_exists   = os.path.exists(timing_csv_path)
                    with open(timing_csv_path, 'a', newline='') as tf:
                        tw = csv.writer(tf)
                        if not timing_exists:
                            tw.writerow([
                                "product_id", "store_id", "seed", "metric",
                                "seq_length", "threshold", "percentile",
                                "ablate_z", "best_epoch",
                ...
                grouped_results = {}
                for (az, p, L), res_dicts in results_by_w_s.items():
                    key = (L,)
                ...
                for (L,), res_dicts in grouped_results.items():
                    ...
                    sub_dir = os.path.join(
                        ...
                        f'seq_{L}', f'item_{product_id}', values_str,
                    )
                    ...
                    emb_title = (
                        f'GCN+MLP (fixed-graph) Forecasts + Ablation ({metric} | Seed={seed} | L={L})'
                    )
                    ...
                    if SAVE_PLOTS:
                        ...
                        plot_results(
                            ...
                            metric=metric, embedding_strategy='gcn_fixed_mlp',
