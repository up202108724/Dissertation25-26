...
# ──────────────────────────────────────────────────────────────────────────
# Recursive inference (FIXED-GRAPH: one graph for the entire horizon)
# ──────────────────────────────────────────────────────────────────────────
@torch.no_grad()
def _recursive_forecast_gcn_fixed(model, ts_seed, fixed_graph,
                                   exog_test_scaled, scaler, horizon, device,
                                   target_history_unscaled=None,
                                   lag_col_indices: Optional[Dict[int, int]] = None,
                                   rolling_mean_excl_col_indices: Optional[Dict[int, int]] = None,
                                   exog_scaler=None):
    """
    One-step-at-a-time inference for the fixed-graph GCN + MLP forecaster.

    The same ``fixed_graph`` (built from the last W days of the train+val
    period) is reused at every forecast step.  The GCN embedding is therefore
    the same across all steps; temporal variation is handled by the rolling
    ``ts_seed`` window.

    Parameters
    ----------
    model            : SimpleGCNMLPForecaster (fixed-graph variant)
    ts_seed          : (L, 1+n_exog) scaled lookback seed; row L-1 already
                       contains the exog for the FIRST predicted step.
    fixed_graph      : single ``torch_geometric.data.Data`` — fixed graph
                       shared across all forecast steps.
    exog_test_scaled : (horizon, n_exog) scaled exog for the test window.
    scaler           : sklearn MinMaxScaler fit on the target.
    horizon          : number of recursive steps.
    device           : torch.device

    Leakage-safe dynamic exog (optional — same contract as the per-step
    variant, see ``_recursive_forecast_gcn_perstep`` for full docs):
    target_history_unscaled, lag_col_indices,
    rolling_mean_excl_col_indices, exog_scaler.
    ...
    # Pre-build the fixed graph batch once and keep it on device.
    fixed_batch = Batch.from_data_list([fixed_graph.clone()]).to(device)
    tidx        = fixed_batch.ptr[:-1].to(device)                 # (1,)

    preds_scaled = []

    for step in range(horizon):
        # advance exog of the last lookback row to the step we are predicting
        if step > 0 and exog_test_scaled is not None and ts.shape[1] > 1:
            ts[-1, 1:] = exog_test_scaled[step]

        # overwrite leaky exog columns from rolling history (if enabled)
        if use_dynamic_exog and ts.shape[1] > 1:
            for col_in_exog, k in lag_col_indices.items():
                raw_lag = float(y_history[-k])
                ts[-1, 1 + col_in_exog] = _scale_lag_value(
        ...
        ts_t = torch.from_numpy(ts).unsqueeze(0).to(device)       # (1, L, F)
        out  = model(fixed_batch, tidx, ts_t)                     # (1, H, 1)
        y_hat = float(out[0, -1, 0].detach().cpu().item())
        preds_scaled.append(y_hat)
