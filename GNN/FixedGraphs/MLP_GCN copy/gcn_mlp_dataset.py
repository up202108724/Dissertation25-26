...
# ── Fixed-graph dataset & collate ──────────────────────────────────────────

class GCNFixedGraphDataset(Dataset):
    """
    Fixed-topology dataset for the FixedGraph GCN + MLP forecaster.

    The same PyG ``Data`` object (fixed graph built from the train+val period)
    is returned for every sample.  The temporal variation is captured entirely
    by ``ts_seq``; the GCN learns to embed the structural neighbourhood context
    from the fixed graph.

    Parameters
    ----------
    target_data     : (T,) scaled target series
    exog_data       : (T, n_exog) scaled exogenous matrix (or None)
    seq_length      : lookback length L
    fixed_pyg_graph : single ``torch_geometric.data.Data`` — the fixed graph
                      shared by all samples (built from the last W days of
                      the train+val period).
    """
    ...
    def __init__(
        ...
        self.seq_length      = int(seq_length)
        self.fixed_pyg_graph = fixed_pyg_graph
        self.has_exog        = self.exog_data is not None
    ...
    def __getitem__(self, idx):
        ...
        ts_tensor = torch.from_numpy(ts.astype(np.float32))
        y_tensor  = torch.tensor([y], dtype=torch.float32)
        # Return the same fixed graph for every sample.
        return self.fixed_pyg_graph, ts_tensor, y_tensor


def collate_fixed_graph_ts(batch):
    """
    Collate for the fixed-graph dataset.  Batches B copies of
    ``(fixed_graph, ts_seq, y)`` into:

        pyg_batch  : torch_geometric.data.Batch of B ego-graphs
                     (one per sample — all share the same topology/features)
        ts_batch   : (B, L, F)
        y_batch    : (B, 1)
        target_idx : (B,) absolute indices of each subgraph's target node
                     inside the batched node-feature matrix (= ptr[:-1]).
        None       : placeholder matching the ``_L`` slot of the per-step
                     collate signature so the training loop is unchanged.
    """
    graphs, ts_seqs, ys = zip(*batch)
    pyg_batch  = Batch.from_data_list(list(graphs))
    ts_batch   = torch.stack(ts_seqs, dim=0)
    y_batch    = torch.stack(ys, dim=0)
    target_idx = pyg_batch.ptr[:-1]                       # (B,)
    return pyg_batch, ts_batch, y_batch, target_idx, None
