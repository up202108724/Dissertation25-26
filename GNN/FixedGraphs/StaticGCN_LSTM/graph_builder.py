...
def build_static_graph(
    ...
    # Risk-3 mitigation: per-feature standardisation so no single statistic
    # dominates the GCN aggregation due to scale differences.
...
def build_ego_graph(
    ...
    threshold: float = 0.82,
    train_end_idx: int = None,  # use only the first train_end_idx date-columns
    include_2hop: bool = False, # also add neighbours-of-neighbours as nodes
) -> tuple:
    """
    Build a PyG ego-graph centred on target_item_id.

    Node set  : [target] + 1-hop neighbours (sim ≥ threshold)
                + optionally 2-hop neighbours (neighbours of 1-hop nodes
                  that are not already in the graph, sim ≥ threshold).
    Edges     : target   ↔ 1-hop neighbour        (weight = similarity)
                1-hop    ↔ 1-hop  if mutual sim ≥ threshold
                1-hop    ↔ 2-hop  if sim ≥ threshold  (only when include_2hop)
    ...
    include_2hop   : if True, extend the graph with 2nd-order neighbours.
    ...
    # --- 3. Select 1-hop neighbours ----------------------------------------
    hop1_global_idxs = [
        i for i in range(len(item_ids))
        if i != target_global_idx and active_mask[i] and sims[i] >= threshold
    ]

    # Risk-1 mitigation: guarantee at least 1 neighbour so the GCN always
    # has something to aggregate from.  Fall back to the most similar active
    # item when no candidate clears the threshold.
    if len(hop1_global_idxs) == 0:
        candidates = [
            (sims[i], i) for i in range(len(item_ids))
            if i != target_global_idx and active_mask[i]
        ]
        if candidates:
            best_sim, best_i = max(candidates, key=lambda t: t[0])
            hop1_global_idxs = [best_i]
            print(f"  [ego_graph] WARNING: no neighbours above threshold={threshold:.2f}; "
                  f"falling back to closest item {item_ids[best_i]} "
                  f"(sim={best_sim:.4f})")

    K1 = len(hop1_global_idxs)

    # Initialise ego-graph node list: target always at index 0
    ego_global_idxs : list = [target_global_idx] + hop1_global_idxs
    in_graph        : set  = set(ego_global_idxs)   # fast membership test
    item_to_node    : dict = {item_ids[g]: local for local, g in enumerate(ego_global_idxs)}

    # --- 4. Build edges for 1-hop ------------------------------------------
    src_list, dst_list, w_list = [], [], []

    # target ↔ 1-hop neighbours
    for local_v, global_i in enumerate(hop1_global_idxs, start=1):
        w = float(sims[global_i])
        src_list += [0, local_v]
        dst_list += [local_v, 0]
        w_list   += [w, w]

    # 1-hop ↔ 1-hop (mutual similarity ≥ threshold)
    if K1 > 1:
        hop1_matrix = ts_matrix[hop1_global_idxs]   # (K1, T)
        if metric == 'spearman':
            sub_sim = _spearman_matrix(hop1_matrix)
        else:
            sub_sim = _pearson_matrix(hop1_matrix)
    ...
    # --- 5. Optional 2-hop expansion ---------------------------------------
    K2 = 0
    if include_2hop and K1 > 0:
        for local_v, global_i in enumerate(hop1_global_idxs, start=1):
            hop1_row  = ts_matrix[global_i]
            hop1_sims = _sim_1vsall(hop1_row, ts_matrix, metric=metric)   # (N,)

            for j in range(len(item_ids)):
                if j in in_graph:
                    continue
                if active_mask[j] and hop1_sims[j] >= threshold:
                    # New 2-hop node — assign next local index
                    local_j = len(ego_global_idxs)
                    ego_global_idxs.append(j)
                    in_graph.add(j)
                    item_to_node[item_ids[j]] = local_j
                    K2 += 1

                    w = float(hop1_sims[j])
                    src_list += [local_v, local_j]
                    dst_list += [local_j, local_v]
                    w_list   += [w, w]

    # --- 6. Node features (computed after node set is finalised) -----------
    x = np.stack(
        [compute_node_features_static(ts_matrix[i]) for i in ego_global_idxs],
        axis=0,
    )
    # Risk-3 mitigation: standardise within the ego-graph node set.
    if x.shape[0] > 1:
        x = StandardScaler().fit_transform(x).astype(np.float32)

    E = len(src_list)
    hop2_str = f" + {K2} 2-hop" if include_2hop else ""
    print(f"  [ego_graph] {len(ego_global_idxs)} nodes "
          f"({K1} 1-hop{hop2_str}) | {E // 2} undirected edges "
