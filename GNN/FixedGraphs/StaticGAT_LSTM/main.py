...
from train import TrainConfig, train_static_gat_lstm
...
# ── Grid ──────────────────────────────────────────────────────────────────────
METRIC               = 'spearman'
SPEARMAN_THRESHOLDS  = [0.5, 0.55, 0.6]
SEEDS                = [42]
...
def main():
    ...
    # Column index in df_wide that marks the end of the training period
    # (= before validation starts, to avoid leakage into graph construction)
    global_val_start_col = L - forecast_horizon - val_size
        ...
        for seed in SEEDS:
            ...
            for threshold in SPEARMAN_THRESHOLDS:
                ...
                (model, trained_scaler,
                 ...
                 graph_data, item_to_node) = train_static_gat_lstm(
                    df                   = df_product_lstm,
                    cfg                  = cfg,
                    seed                 = seed,
                    loss_type            = 'mse',
                    ...
                    metric               = METRIC,
                    threshold            = threshold,
                    train_end_idx_global = global_val_start_col,
