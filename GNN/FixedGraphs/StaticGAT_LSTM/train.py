"""
train.py — Training loop for the Static-GAT + LSTM forecaster.
...
from gat_lstm_pyg import StaticGATLSTMForecaster
from dataset import StaticGCNDataset, make_windows
from graph_builder import build_static_graph, NODE_FEAT_DIM
...
def train_static_gat_lstm(
    ...
    # ── Build the global static graph (once) ───────────────────────────────
    graph_data, item_to_node = build_static_graph(
        df_wide         = df_wide,
    ...
    model = StaticGATLSTMForecaster(
        in_channels     = NODE_FEAT_DIM,
        gat_hidden      = gcn_hidden,
        gat_out         = gcn_out,
    ...
    print(f"  GAT in={NODE_FEAT_DIM} hidden={gcn_hidden} out={gcn_out} | "
    ...
    best_model_path = os.path.join(model_dir, f'static_gat_lstm_{product_id}.pth')
