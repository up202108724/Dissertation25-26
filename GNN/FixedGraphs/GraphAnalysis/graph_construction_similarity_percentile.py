...
def neighbourhood_graph(product_id, df, similarity_metric, window_size, threshold=None, percentile=None, step_size=1, cat_labels=None, plot_dir=None):
    """
    ...
    for start_idx in range(0, time_steps - window_size + 1, step_size):
        ...
        if percentile is not None:
            # We want the highest similarity, so we look at the upper percentile (e.g. 100 - x)
            # If percentile=5 is passed, it means top 5%, so we want the 95th percentile threshold
            current_threshold = np.percentile(valid_similarities, 100 - percentile)
        else:
            current_threshold = threshold
        
        mask = valid_similarities >= current_threshold
        ...
        if plot_dir is not None and len(G.nodes) > 1:
            ...
            plot_networkx_plotly(
                G, 
                title=f"Graph from {start_date} to {end_date} (Method: {similarity_metric}, Threshold: {current_threshold:.4f})", 
                save_path=save_path,
    ...
    for config in grid_configs:
        ...
        for th in thresholds:
            for pct in percentiles:
                ...
                print(f"Finished {metric} th={th} pct={pct}! Out of {len(graphs)} windows, {len(valid_graphs)} had valid neighbors.")
