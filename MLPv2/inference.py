...
def recursive_inference(
    ...
    """
    Recursive 1-step forecast with STATIC exogenous features.

    WARNING: this version assumes every exogenous column is "static" w.r.t. the
    target (calendar / holiday / promo) — i.e. its value at step t does not
    depend on past values of y. If your `future_exog` contains lag_* or
    rolling_mean_* columns computed on the ground-truth test target, those
    values WILL LEAK. Use `recursive_inference_dynamic_exog` instead.
    """
