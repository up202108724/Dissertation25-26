    ...
    with open(inf_log_path, 'w') as inf_log_file:
        ...
        with torch.no_grad():
            for step in range(forecast_window):
                ...
                # MLP outputs (b, horizon, out_dim) -> Since horizon defaults to 1 here, index [0, 0, 0]
                pred = model(x).cpu().numpy()[0, 0, 0]
    ...
    for seed in seeds:
        for item_id, store_id in products:
            ...
            for strategy in strategies:
                ...
                # Instantiate MLP rather than LSTM
                model = MLPForecaster(
                    lookback=lookback_window,
                    in_channels=input_size,
                    horizon=1, # single step recursive inference
                    out_dim=1,
